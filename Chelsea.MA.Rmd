---
title: "Municipal Indicators For Chelsea, MA"
output:
  pdf_document:
    toc: yes
    toc_depth: '3'
...
In this report, 41 demographic, educational institutions, employment, healthcare, municipal finance, infrastructure, food insecurity and housing variables are presented using federally-published data sources for the city of Chelsea, MA. To better demonstrate their ties to geography of the city, the variables have been mapped on the block group level. \
...
```
* The median household income in the past 12 months is 53280 Dollar in `r Chelsea.MA$City`.\
...
library(tmap)
tm_shape(chelsea.sex) + tm_polygons(c('Female', 'Male'), palette=list("Purples", "Reds"), title = c("Female","Male"))  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")+ tmap_options(show.messages = FALSE) 

make_percent <- function(x, digits = 2) {
  paste0("%", formatC(x, digits = digits, format = "f", big.mark = ","))
}
tm_shape(chelsea.sex) + tm_polygons(c('FemalePer', 'MalePer'), palette=list("Blues", "Oranges"), title = c("Female","Male"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")+ tmap_options(show.messages = FALSE) 
...
* `r Chelsea.MA$City` has 3 public school district, which contains 6,289 students and 11 public schools. Also, `r  Chelsea.MA$City` has 1 private schools with 28 students.(https://nces.ed.gov/) 
...
Chelsea.MA$SchoolsDistStudents <- 6289
Chelsea.MA$PubSchools <- 11
...
* `r Chelsea.MA$City` has no university in 02150 ZIP Code.(https://nces.ed.gov/) 
...
* Chelsea School District in `r Chelsea.MA$City`contains 413 teachers and 290 other staffs. The total number of teachers in all private schools in `r Chelsea.MA$City` is 7.(http://www.usa.com/)
...
```
* The number of labor force, who are 16 years and older in `r Chelsea.MA$City` is 21754 which is `r round(Chelsea.MA$LabForcePer,2)` of total number of labors of which 21754 are civilian labor forces and `r Chelsea.MA$Military` are armed labor forces.\
...
tm_shape(chelsea.emp) + tm_polygons(c("EmpRate_BG","UnempRate_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("Employment rate","Unemployment rate")) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
...
tm_shape(chelsea.majorEmp) + tm_polygons(c("MaleNCAISPer_BG","FemaleNCAISPer_BG"), style="jenks", palette=list("Blues","Reds"), border.col = "darkgray", title=c("MaleNCAIS Proportion","FemaleNCAIS Proportion")) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")

tm_shape(chelsea.majorEmp) + tm_polygons(c("MaleNCAIS_BG","FemaleNCAIS_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("MaleNCAIS","FemaleNCAIS")) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
...
```
* The median earning in `r Chelsea.MA$City` is 29158 Dollar.\
...
Chelsea.MA$Hospitals <- 0
```


* `r Chelsea.MA$City` has no hospital.\
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$NursingHomes <- 5
```


* `r Chelsea.MA$City` has 5 nursing homes. (https://nursinghomedirectory.com/)
...
Chelsea.MA$TotalRevenueFY19 <- 181767282
Chelsea.MA$RevenueTaxesFY19 <- 79454275
Chelsea.MA$StateRevenueFY19 <- 89076796 
Chelsea.MA$RevenueMiscellaneousFY19 <- 1547731
Chelsea.MA$RevenueServiceChargesFY19 <- 2542485
Chelsea.MA$RevenuePermitsFY19 <- 3618689
Chelsea.MA$RevenueFineForfeituresFY19 <- 2529796
Chelsea.MA$RevenueGovernmentsFY19 <- 5573
Chelsea.MA$RevenueTransferFY19 <- 2666393
```
* The total revenues for `r Chelsea.MA$City` is $181,767,282 in Fiscal Year 2019, which includes taxes with 79,454,275 Dollar, state revenue with 89,076,796 Dollar, miscellaneous with 1,547,731 Dollar, service charges with 2,542,485 Dollar, licenses and permits with 3,618,689 Dollar, fines and forfeitures with 2,529,796 Dollar, revenue from other governments with 5,573 Dollar, and transfers with 2,666,393 Dollar.(https://dlsgateway.dor.state.ma.us/)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$ExpendituresFY19 <- 170344706
Chelsea.MA$EXEducationFY19 <- 92153553
Chelsea.MA$EXFixedCostsFY19 <- 16899018	
Chelsea.MA$EXDebtServiceFY19 <- 4157494
Chelsea.MA$EXPoliceFY19 <- 12165647
Chelsea.MA$EXPublicWorksFY19 <- 8228601
Chelsea.MA$EXFireFY19 <- 10185138
Chelsea.MA$EXIntergovernmentalFY19 <- 16279647	
Chelsea.MA$EXGeneralGovernmentFY19 <- 5191510
Chelsea.MA$EXHumanServicesFY19 <- 1337198
Chelsea.MA$EXCultureRecreationFY19 <- 709325
Chelsea.MA$EXOtherExpendituresFY19 <- 0
Chelsea.MA$EXPublicSafetyFY19 <- 3037575
```

* The total amount of expenditures for `r Chelsea.MA$City` is $170,344,706 in Fiscal Year 2019 and the major expense driver is related to education, which is 92,153,553 Dollar. After that fixed costs with 16,899,018 Dollar, intergovernmental assessments with 16,279,647 Dollar, police with 12,165,647 Dollar, fire with 10,185,138 Dollar, public works with 8,228,601 Dollar, general government with 5,191,510 Dollar,debt service with 4,157,494 Dollar, other public safety with 3,037,575 Dollar, human services with 1,337,198 Dollar, culture and recreation with 709,325 Dollar, and other expenditures with 0 Dollar. (https://dlsgateway.dor.state.ma.us/)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$ExpendituresFY20 <- 181486465
Chelsea.MA$EXEducationFY20 <- 96931539
Chelsea.MA$EXGeneralGovernmentFY20 <- 5955988

Chelsea.MA$TotalRevenueFY20 <- 181486465
Chelsea.MA$RevenuePopertyTaxFY20 <- 4412938

```


* Expenditures: The proposed FY20 municipal operating budget in `r Chelsea.MA$City` is 181,486,465 Dollar reflecting increase over FY19. The major expense driver is related to education, which is 96,931,539 Dollar. Revenue: The proposed FY20 municipal operating budget in `r Chelsea.MA$City` is 181,486,465 Dollar. Amounts to be raised from the property tax equals 4,412,938 Dollar.\
...
* The current Chelsea Commuter Rail station, on the Newburyport/Rockport Line at Arlington Street in Chelsea, is not accessible to people with disabilities, and is in disrepair.There is no Airport in Chelsea. (https://www.mbta.com/projects/chelsea-commuter-rail-station)
...
* Chelsea is served by the MBTA to make transportation easier for its community. It has seven bus routes and one commuter rail spread throughout the city.(https://www.chelseama.gov/)
...
* "Chelsea has only two major Internet technology choices to grapple with — cable (98.96% availability) and DSL (close to one hundred % coverage). Coaxial cable Internet access is usually provided via cable TV corporations via pre-existing coaxial copper TV cables on utility poles. DSL, similarly, is sent via twisted copper phone wires. The majority of Chelsea census blocks can access cable or DSL from at least two providers. Coverage from Viasat Internet (formerly Exede) and HughesNet, in most cases, will overlap. As a result, the choice between Viasat and HughesNet is common. Chelsea residents generally have two or more Internet providers to them. That said, this number tends to change from block to block. There are 18 internet providers in Chelsea with 7 of those offering residential service." Chelsea is the 193rd most connected city in Massachusetts and the fastest zip code in Chelsea for May 2020 is 02150.(https://broadbandnow.com/Massachusetts/Chelsea)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$NumGrocery <- 180
```
* `r Chelsea.MA$City` has 180 Grocery Stores.\
...
```
* The median house value in `r Chelsea.MA$City` is 323400 Dollar.\

* The average house value in `r Chelsea.MA$City` is 1232714300 Dollar.\
...
tm_shape(chelsea.housing) + tm_polygons("MedianHValue_BG" , palette="Blues", style = "quantile", title = "Median Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") 
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(chelsea.housing) + tm_polygons("AvgHValue_BG" , palette="Greens", style = "quantile", title = "Average Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray")
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(chelsea.housing) + tm_polygons("RentersToOwners_BG" , palette="Reds", style = "quantile", title = "RentersToOwners") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") 
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$PropertyTaxRate.RES <- 13.39
Chelsea.MA$PropertyTaxRate.Com <- 27.16
```
* The amount of tax rate in `r Chelsea.MA$City` is $13.39 per thousand Dollar for residential properties and is 27.16 Dollar for per 1,000 Dollar for commercial properties in fiscal year 2020.(https://www.chelseama.gov/)\
...
tm_shape(chelsea.dur) + tm_polygons(c("MedYrMovedIn_BG","MedYrRentMovedIn_BG","MedYrOwnMovedIn_BG"), style="jenks", palette=list("Greens","seq","Blues"), border.col = "darkgray", title=c("MedYrMovedIn","MedYrRentMovedIn","MedYrOwnMovedIn")) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$Homelessness <- 14
```


* The number of people homeless in 2016 in `r Chelsea.MA$City` is 14 individuals. (https://chelseapolice.com/2017/01/23/chelsea-participates-in-homeless-count-january-25-26-2017/)
...
* "The Department of Planning and Development has established an Emergency Rental Assistance Program. Prior to the pandemic, the crisis of housing affordability heavily impacted Chelsea residents. The City is committed to supporting residents during this period of economic hardship, which has worsened the crisis of housing affordability. As a step to reduce displacement and stabilize housing situations, the City has created this program to offer direct grant funding to residents. Qualifying applicants may be eligible for a grant equivalent to up to three months of rent, or $5,000, whichever amount is less".(https://www.chelseama.gov/)
