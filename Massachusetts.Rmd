---
title: "Indicators For Massachusetts"
output:
  pdf_document:
    toc: yes
    toc_depth: '3'
...
In this report, 30 demographic, educational institutions, employment, municipal finances, food insecurity and housing variables are presented using federally-published data sources for Massachusetts state. To better demonstrate their ties to geography of the state, the variables have been mapped on the block group level. \
...
The total population of `r Each_State$State[1]` is 6830193. Followed is the popuation size map of the state.\
\

```{r , echo=FALSE, warning=FALSE, message=FALSE, comment=""}
## MAP
Pop.BG.MA <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Pop.BG.MA) + tm_polygons('TotalPop_BG' , palette="seq", style = "jenks", title = "Total Population")  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")
```
\
...
```
* The number of people, who are younger than 18 Years old is 1379897 in `r Each_State$State[1]` 
, which is `r (round(Each_State$AgeU18Per_State[1],2))*100`% of total population in `r Each_State$State[1]`. \
* The number of people, who are between 18 and 34 years old is 1665143 in `r Each_State$State[1]`, which is `r (round(Each_State$Age1834Per_State[1],2))*100`% of total population in `r Each_State$State[1]`.\
* The number of people, who are between 35 and 64 years old is 2706929 in `r Each_State$State[1]`, which is `r (round(Each_State$Age3564Per_State[1],2))*100`% of total population in `r Each_State$State[1]`.\
* The number of people, who are older than 65 years old is 1078224 in `r Each_State$State[1]`, which is `r (round(Each_State$AgeO65Per_State[1],2))*100`% of total population in `r Each_State$State[1]`.\

Followed is the popuation age map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## Map
Age.BG.MA <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
tm_shape(Age.BG.MA) + tm_polygons(c('AgeU18Per_BG','Age1834Per_BG',"Age3564Per_BG","AgeO65Per_BG") , palette=list("Oranges", "Greens", "Blues", "Purples") ,style = "quantile", title = c('AgeU18','Age1834',"Age3564","AgeO65"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray") + tmap_options(show.messages = FALSE)  + tm_layout(legend.position = c("left","bottom"))
...
```
* The median household income in the past 12 months is 77378 Dollar in `r Each_State$State[1]`.\
Followed is the Median Household Income map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Income.BG.MA <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
```
* The number of people who are white in `r Each_State$State[1]` is 4930412 which is `r (round(Each_State$WhitePer_State[1],2))*100`% of total population in `r Each_State$State[1]`.\
* The number of people who are black in `r Each_State$State[1]` is 463796 which is `r (round(Each_State$BlackPer_State[1],2))*100`% of total population in `r Each_State$State[1]`.\
* The number of people who are asian in `r Each_State$State[1]` is 440336 which is `r (round(Each_State$AsianPer_State[1],2))*100`% of total population in `r Each_State$State[1]`.\
* The number of people who are hispanic in `r Each_State$State[1]` is 789127 which is `r (round(Each_State$HispanicPer_State[1],2))*100`% of total population in `r Each_State$State[1]`.\
* The number of people who are have two or more races in `r Each_State$State[1]` is 142666 which is `r (round(Each_State$TwoOrMorePer_State[1],2))*100`% of total population in `r Each_State$State[1]`. \
Followed is the Racial Breakdown map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## Map
Race.BG.MA <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
```
* The number of females in `r Each_State$State[1]` is 3516214 which is `r 100*(round(Each_State$FemalePer_State[1],2))`% of total population in `r Each_State$State[1]`.\
* The number of males in `r Each_State$State[1]` is 3313979 which is `r 100*(round(Each_State$MalePer_State[1],2))`% of total population in `r Each_State$State[1]`.\
* The ratio of number of females to males in `r Each_State$State[1]` is `r round(Each_State$SexRatio_State[1],2)`.\
Followed is the Gender Breakdown map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Sex.BG.MA <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Sex.BG.MA) + tm_polygons(c('Female', 'Male'), palette=list("Purples", "Reds"), title = c("Female","Male"))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")+ tmap_options(show.messages = FALSE) + tm_layout(legend.position = c("left","bottom"))
```
...
library(tmap)
tm_shape(Sex.BG.MA) + tm_polygons(c('FemalePer', 'MalePer'), palette=list("Blues", "Oranges"), title = c("Female","Male"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")+ tmap_options(show.messages = FALSE) + tm_layout(legend.position = c("left","bottom"))
...
```
* The average household size is `r round(Each_State$AveHouseH_State[1], 2)` in `r Each_State$State[1]`.\
Followed is the Average Household Size map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
HH.size.BG.MA <- tidycensus::get_acs(geography = "block group",
															survey="acs5",
															state=i,
															year = endyr,
															output = "wide",
...
tm_shape(HH.size.BG.MA) + tm_polygons("AveHouseH_BG", style = "quantile", palette="Purples", border.col = "darkgray", title="Ave Household Size") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite") + tmap_options(show.messages = FALSE)
...
```
* The number of people, who have less than high school degree is 454360 in `r Each_State$State[1]` which is `r (round(Each_State$LessThanHSPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State$State[1]`.\

* The number of people, who have high school degree is 1150846 in `r Each_State$State[1]` which is `r (round(Each_State$HSGradPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State$State[1]`.\

* The number of people, who have some college degree is 1105887 in `r Each_State$State[1]` which is `r (round(Each_State$SomeCollPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State$State[1]`.\

* The number of people, who have bachelor degree is 1128877 in `r Each_State$State[1]` which is `r (round(Each_State$BachPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State$State[1]`.\

* The number of people, who have master's degree is 637628 in `r Each_State$State[1]` which is `r (round(Each_State$MasterPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State$State[1]`.\

* The number of people, who have professional school degree is 141903 in `r Each_State$State[1]` which is `r (round(Each_State$ProfPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State$State[1]`.\

* The number of people, who have doctorate degree is 129294 in `r Each_State$State[1]` which is `r (round(Each_State$DocPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State$State[1]`.\
Followed is the Educational Attainment map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Educ.BG.MA <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
```
* The percentage of single-female-headed is `r (round(Each_State$FemHeadPer_State[1],2))*100`% of total number of family households in `r Each_State$State[1]`.\
* The percentage of single-male-headed is `r (round(Each_State$MaleHeadPer_State[1],2))*100`% of total number of family households in `r Each_State$State[1]`.\
Followed is the Single Parent Households map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Single.HH.BG.MA <- tidycensus::get_acs(geography = "block group",
															survey="acs5",
															state=i,
															year = endyr,
															output = "wide",
...
```
* The percentage of family, who are in poverty is `r (round(Each_State$FamPovPer_State[1],2))*100` % in `r Each_State$State[1]`.\
* The percentage of population, who are in poverty is `r (round(Each_State$PopPovPer_State[1],2))*100` % in `r Each_State$State[1]`.\
* The total number of people, who are in poverty is 710305 in `r Each_State$State[1]`.\
Followed is the Population in Poverty map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Poverty.BG.MA <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Poverty.BG.MA) + tm_polygons("FamPovPer_BG", style="jenks", palette="Purples", border.col = "darkgray", title="Family in Poverty",popup.format = list(fun = make_percent), legend.format = list(fun = make_percent)) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("left","bottom"))
...
## GIVE US NA
```
* The percentage of householders, who are living alone and are older than 65 years old is `r (round(Each_State$OldAlonePer_State[1],2))*100` % in `r Each_State$State[1]`.\
...
* `r Each_State$State[1]` has 432 public school district, which contain 1853 public schools. Also, `r Each_State$State[1]` has 517 private schools.(https://nces.ed.gov/)

\


```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State$SchoolsDistrict <- 432
Each_State$PubSchools <- 1853
Each_State$PvSchools <- 517
```
...
* `r Each_State$State[1]` has 151 universities.(https://nces.ed.gov/)

```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State$NumUni <- 151
```
...
```
* The total number of students, who enrolled in preschools to 12 grade levels in `r Each_State$State[1]` schools is 1144323.\
...
```
* The number of jobs within community boundaries in `r Each_State$State[1]` is 3362070.\

Followed is the map of Number of Jobs within Community Boundaries of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}

## MAP
Num.jobs.BG.MA <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Num.jobs.BG.MA) + tm_polygons("NumJobs_BG", style="jenks", palette="Reds", border.col = "darkgray", title="Number of Jobs") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
```
* The number of people, who work in state of residence and worked in county of residence is 2282885 in `r Each_State$State[1]`, which is `r (round(Each_State$WorkStateCountyPer_State[1],2))*100` % of total workers in `r Each_State$State[1]`.\
\
...
```
* The number of labor force, who are 16 years and older in `r Each_State$State[1]` is 3778642 which is `r round(Each_State$LabForcePer_State[1],2)` of total number of labors of which 3773721 are civilian labor forces and 4921 are armed labor forces.\

Followed is the map of Labor Force of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Labor.Force.BG.MA <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Labor.Force.BG.MA) + tm_polygons("LabForce_BG", style="jenks", palette="seq", border.col = "darkgray", title="Number of Labors") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
```
* The rate of employment in `r Each_State$State[1]` is `r round(Each_State$EmpRate_State[1],2)*100`%.\
* The rate of unemployment in `r Each_State$State[1]` is `r round(Each_State$UnempRate_State[1],2)*100`%.\

Followed is Employed and Unemployed map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Emp.Unemp.BG.MA <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Emp.Unemp.BG.MA) + tm_polygons(c("EmpRate_BG","UnempRate_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("Employment rate","Unemployment rate")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("left","bottom"))
...
```
* Number of male by industry for the civilian employed in `r Each_State$State[1]` is 1809718 which is `r round(Each_State$MaleNCAISPer_State[1],2)` of population, who are 16 years and over.\
* Number of female by industry for the civilian employed in `r Each_State$State[1]` is 1760539 which is `r round(Each_State$FemaleNCAISPer_State[1],2)` of population, who are 16 years and over.\

Followed is the map of the Major Employers by NCAIS Code of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Emp.NCAIS.BG.MA <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Emp.NCAIS.BG.MA) + tm_polygons(c("MaleNCAIS_BG","FemaleNCAIS_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("MaleNCAIS","FemaleNCAIS")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") +tm_layout(legend.position = c("left","bottom"))
...
```
* The percentage of people, who are in agriculture, forestry, fishing, hunting, and mining in `r Each_State$State[1]` is `r (round(Each_State$Agriculture_State[1],2))*100`%.\

* The percentage of people, who are in construction in `r Each_State$State[1]` state is `r (round(Each_State$Construction_State[1],2))*100`%".\

* The percentage of people, who are in manufacturing in `r Each_State$State[1]` state is `r (round(Each_State$Manufacturing_State[1],2))*100`%.\

* The percentage of people, who are in wholesale trade in `r Each_State$State[1]` state is `r (round(Each_State$WholesaleTrade_State[1],2))*100`%.\

* The percentage of people, who are in retail trade in `r Each_State$State[1]` state is `r (round(Each_State$RetailTrade_State[1],2))*100`%.\

* The percentage of people, who are in transportation and warehousing, and utilities in`r Each_State$State[1]`state is`r (round(Each_State$Transportation_State[1],2))*100`%.\

* The percentage of people, who are in information in `r Each_State$State[1]` state is `r (round(Each_State$Information_State[1],2))*100`%.\

* The percentage of people, who are in finance and insurance, and real estate, and rental and leasing in `r Each_State$State[1]` state is `r (round(Each_State$Finance_Ins_State[1],2))*100`%.\

* The percentage of people, who are in professional, scientific, and management, and administrative, and waste management services in `r Each_State$State[1]` state is `r (round(Each_State$Professional_State[1],2))*100`%.\

* The percentage of people, who are in educational services, and health care and social assistance in `r Each_State$State[1]` state is `r (round(Each_State$Educ_Health_SocSvcs_State[1],2))*100`%.\

* The percentage of people, who are in arts, entertainment, and recreation, and accommodation and food services in `r Each_State$State[1]` state is `r (round(Each_State$RecreationEtc_State[1],2))*100`%.\

* The percentage of people, who are in other services, except public administration in `r Each_State$State[1]` state is `r (round(Each_State$OtherIndustries_State[1],2))*100`%.\

* The percentage of people, who are in Public administration in `r Each_State$State[1]` state is `r (round(Each_State$PublicAdmin_State[1],2))*100`%.\
\
...
```
* The median earning in `r Each_State$State[1]` is 41476 Dollar`.\
Followed is the Median Earning map of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
# Earning at Block Group level
Med.Earning.BG.MA <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Med.Earning.BG.MA) + tm_polygons("MedEarning_BG" , palette="seq", style = "jenks", title = "Median Earning", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") 
...
```
* The number of workers, who are younger than 29 years old in `r Each_State$State[1]` is 874627 which is `r (round(Each_State$AgeEmpU29Per_State[1],2))*100`% of total number of workers.\

* The number of workers, who are between 30 and 54 years old in `r Each_State$State[1]` is 1838732 which is `r (round(Each_State$AgeEmp3054Per_State[1],2))*100`% of total number of workers.\

* The number of workers, who are older than 55 years old in `r Each_State$State[1]` is 861819 which is `r (round(Each_State$AgeEmpO55Per_State[1],2))*100`% of total number of workers.\
...
* The number of white workers in `r Each_State$State[1]` is 2868991 which is `r (round(Each_State$WhiteEmpPer_State[1],2))*100`% of total number of workers.\

* The number of black workers in `r Each_State$State[1]` is 250461 which is `r (round(Each_State$BlackEmpPer_State[1],2))*100`% of total number of workers.\

* The number of asian workers in `r Each_State$State[1]` is 230121 which is `r (round(Each_State$AsianEmpPer_State[1],2))*100`% of total number of workers.\

* The number of hispanic or latino workers in `r Each_State$State[1]` is 352931 which is `r (round(Each_State$HispanicEmpPer_State[1],2))*100`% of total number of workers.\

* The number of workers, who have two or more races in `r Each_State$State[1]` is 83440 which is `r (round(Each_State$TwoOrMoreEmpPer_State[1],2))*100`% of total number of workers.\
\
...
```
* The number of workers, who have less than high school degree in `r Each_State$State[1]` is 191193 which is `r (round(Each_State$LessThanHSLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have high school degree in `r Each_State$State[1]` is 685199 which is `r (round(Each_State$HSGradLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have some college or associates degree in `r Each_State$State[1]` is 774160 which is `r (round(Each_State$SomeCollLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have bachelors degree or higher in `r Each_State$State[1]` is 1594678 which is `r (round(Each_State$BachOrHigherLaborPer[1],2))*100`% of total number of workers.\
...
```
* The number of male workers in `r Each_State$State[1]` is 1814066 which is `r (round(Each_State$EmpMalePer[1],2))*100`% of total number of workers.\
* The number of female workers in `r Each_State$State[1]` is 1761112 which is `r (round(Each_State$EmpFemalePer[1],2))*100`% of total number of workers.\
...
Each_State$TotalRevenueFY19 <- 26249128982
Each_State$RevenueTaxesFY19 <- 18556467445
Each_State$StateRevenueFY19 <- 5580318267 
Each_State$RevenueMiscellaneousFY19 <- 280237698
Each_State$RevenueServiceChargesFY19 <- 580362920
Each_State$RevenuePermitsFY19 <- 427111126
Each_State$RevenueFineForfeituresFY19 <- 113382205
Each_State$RevenueGovernmentsFY19 <- 19878980
Each_State$RevenueTransferFY19 <- 560519790
```
* The total revenues for `r Each_State$State[1]` is $26,249,128,982 in Fiscal Year 2019, which includes taxes with 18,556,467,445 Dollar, state revenue with 5,580,318,267 Dollar, miscellaneous with 280,237,698 Dollar, service charges with 580,362,920 Dollar, licenses and permits with 427,111,126 Dollar, fines and forfeitures with 113,382,205 Dollar, revenue from other governments with 19,878,980 Dollar, and transfers with 560,519,790 Dollar.(https://dlsgateway.dor.state.ma.us/)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State$ExpendituresFY19 <- 24422219209
Each_State$EXEducationFY19 <- 11433143380
Each_State$EXFixedCostsFY19 <- 3413994372
Each_State$EXDebtServiceFY19 <- 1492409367
Each_State$EXPoliceFY19 <- 1847818125
Each_State$EXPublicWorksFY19 <- 1341242990
Each_State$EXFireFY19 <- 1382254114
Each_State$EXIntergovernmentalFY19 <- 973022907
Each_State$EXGeneralGovernmentFY19 <- 1315715966
Each_State$EXHumanServicesFY19 <- 380563682
Each_State$EXCultureRecreationFY19 <- 472667897
Each_State$EXOtherExpendituresFY19 <- 45112496
Each_State$EXPublicSafetyFY19 <- 324273913
```

* The total amount of expenditures for `r Each_State$State[1]` is $24,422,219,209 in Fiscal Year 2019 and the major expense driver is related to education, which is 11,433,143,380 Dollar. Also, expenditures include fixed costs with 3,413,994,372 Dollar, debt service with 1,492,409,367 Dollar, police with 1,847,818,125 Dollar, public works with 1,341,242,990 Dollar, fire with 1,382,254,114 Dollar, intergovernmental assessments with 973,022,907 Dollar, general government with 1,315,715,966 Dollar, human services with 380,563,682 Dollar, culture and recreation with 472,667,897 Dollar, other expenditures with 45,112,496 Dollar, and other public safety with 324,273,913 Dollar. (https://dlsgateway.dor.state.ma.us/)
...
```
* The percent of households, who received Food Stamps/SNAP in the past 12 months in `r Each_State$State[1]` state is `r (round(Each_State$SnapRec_State[1],2))*100`%.\
...
```
* The median house value in `r Each_State$State[1]` is 366800 Dollar.\

* The average house value in `r Each_State$State[1]` is 447355 Dollar.\

* The proportion of renters to owners in `r Each_State$State[1]` is `r round(Each_State$RentersToOwners_State[1],2)`.\

* The percentage of renters in `r Each_State$State[1]` is `r (round(Each_State$RentersPer_State[1],2))*100`%.\

* The percentage of owners in `r Each_State$State[1]` is `r (round(Each_State$HomeOwnPer_State[1],2))*100`%.\

Followed is the map of Proportion of Renters to Owners, Median Home Values, and Average Home Values of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Housing.BG.MA <- tidycensus::get_acs(geography = "block group",
															 survey="acs5",
															 state=i,
															 year = endyr,
															 output = "wide",
...
tm_shape(Housing.BG.MA) + tm_polygons("MedianHValue_BG" , palette="Blues", style = "quantile", title = "Median Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("left","bottom"))
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(Housing.BG.MA) + tm_polygons("AvgHValue_BG" , palette="Greens", style = "quantile", title = "Average Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("left","bottom"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(Housing.BG.MA) + tm_polygons("RentersToOwners_BG" , palette="Reds", style = "jenks", title = "RentersToOwners") + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("left","bottom"))
```
...
```
* The median year that a homeowner moved into their current domicile in `r Each_State$State[1]` is `r Each_State$MedYrOwnMovedIn_State[1]`.\
* The median year that a renter moved into their current domicile in `r Each_State$State[1]` is `r Each_State$MedYrRentMovedIn_State[1]`.\
Followed is the map of Duration of Occupancy of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Dur.Occ.BG.MA <- tidycensus::get_acs(geography = "block group",
															 survey="acs5",
															 state=i,
															 year = endyr,
															 output = "wide",
...
tm_shape(Dur.Occ.BG.MA) + tm_polygons(c("MedYrMovedIn_BG","MedYrRentMovedIn_BG","MedYrOwnMovedIn_BG"), style="jenks", palette=list("Greens","seq","Blues"), border.col = "darkgray", title=c("MedYrMovedIn","MedYrRentMovedIn","MedYrOwnMovedIn")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("left","bottom"))
```
...
```
* The number of households, who spend 30 percent or more on housing costs in `r Each_State$State[1]` is 443059 which is `r (round(Each_State$CostBurdenedPer[1],3))*100`% of total number of households, who spend money on housing costs.\
