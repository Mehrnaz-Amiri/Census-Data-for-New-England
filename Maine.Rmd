...
In this report, 30 demographic, educational institutions, employment, municipal finances, food insecurity and housing variables are presented using federally-published data sources for Maine state. To better demonstrate their ties to geography of the state, the variables have been mapped on the block group level. \
...
Each_State3 <- Pop.State
```


The total population of `r Each_State3$State[1]` is 1332813. Followed is the popuation size map of the state.\
\

```{r , echo=FALSE, warning=FALSE, message=FALSE, comment=""}
## MAP
Pop.BG.ME <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Pop.BG.ME) + tm_polygons('TotalPop_BG' , palette="seq", style = "jenks", title = "Total Population")  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")
```
\
...
Age.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, by="BG_ID_10") %>%
...
Each_State3 <- Age.State
```
* The number of people, who are younger than 18 Years old is 254661 in `r Each_State3$State[1]` 
, which is `r (round(Each_State3$AgeU18Per_State[1],2))*100`% of total population in `r Each_State3$State[1]`. \
* The number of people, who are between 18 and 34 years old is 266101 in `r Each_State3$State[1]`, which is `r (round(Each_State3$Age1834Per_State[1],2))*100`% of total population in `r Each_State3$State[1]`.\
* The number of people, who are between 35 and 64 years old is 552875 in `r Each_State3$State[1]`, which is `r (round(Each_State3$Age3564Per_State[1],2))*100`% of total population in `r Each_State3$State[1]`.\
* The number of people, who are older than 65 years old is 259176 in `r Each_State3$State[1]`, which is `r (round(Each_State3$AgeO65Per_State[1],2))*100`% of total population in `r Each_State3$State[1]`.\

Followed is the popuation age map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## Map
Age.BG.ME <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Age.BG.ME) + tm_polygons(c('AgeU18Per_BG','Age1834Per_BG',"Age3564Per_BG","AgeO65Per_BG") , palette=list("Oranges", "Greens", "Blues", "Purples") ,style = "quantile", title = c('AgeU18','Age1834',"Age3564","AgeO65"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray") + tmap_options(show.messages = FALSE) + tm_layout(legend.position = c("right","bottom"))
...
Each_State3 <- Income.State
```
* The median household income in the past 12 months is 55425 Dollar in `r Each_State3$State[1]`.\
Followed is the Median Household Income map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Income.BG.ME <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
Race.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Race.BG, by="BG_ID_10") %>%
...
Each_State3 <- Race.State
```
* The number of people who are white in `r Each_State3$State[1]` is 1244525 which is `r (round(Each_State3$WhitePer_State[1],2))*100`% of total population in `r Each_State3$State[1]`.\
* The number of people who are black in `r Each_State3$State[1]` is 16990 which is `r (round(Each_State3$BlackPer_State[1],2))*100`% of total population in `r Each_State3$State[1]`.\
* The number of people who are asian in `r Each_State3$State[1]` is 14814 which is `r (round(Each_State3$AsianPer_State[1],2))*100`% of total population in `r Each_State3$State[1]`.\
* The number of people who are hispanic in `r Each_State3$State[1]` is 21421 which is `r (round(Each_State3$HispanicPer_State[1],2))*100`% of total population in `r Each_State3$State[1]`.\
* The number of people who are have two or more races in `r Each_State3$State[1]` is 26240 which is `r (round(Each_State3$TwoOrMorePer_State[1],2))*100`% of total population in `r Each_State3$State[1]`. \
Followed is the Racial Breakdown map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## Map
Race.BG.ME <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
Sex.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Sex.BG, by="BG_ID_10") %>%
...
Each_State3 <- Sex.State
```
* The number of females in `r Each_State3$State[1]` is 680514 which is `r 100*(round(Each_State3$FemalePer_State[1],2))`% of total population in `r Each_State3$State[1]`.\
* The number of males in `r Each_State3$State[1]` is 652299 which is `r 100*(round(Each_State3$MalePer_State[1],2))`% of total population in `r Each_State3$State[1]`.\
* The ratio of number of females to males in `r Each_State3$State[1]` is `r round(Each_State3$SexRatio_State[1],2)`.\
Followed is the Gender Breakdown map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Sex.BG.ME <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Sex.BG.ME) + tm_polygons(c('FemalePer', 'MalePer'), palette=list("Blues", "Oranges"), title = c("Female","Male"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")+ tmap_options(show.messages = FALSE) + tm_layout(legend.position = c("right","bottom"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
library(tmap)
tm_shape(Sex.BG.ME) + tm_polygons(c('Female', 'Male'), palette=list("Purples", "Reds"), title = c("Female","Male"))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")+ tmap_options(show.messages = FALSE) + tm_layout(legend.position = c("right","bottom"))
```
\
...
HH.size.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, HH.size.BG, by="BG_ID_10")
...
Each_State3 <- HH.size.State
```
* The average household size is `r round(Each_State3$AveHouseH_State[1], 2)` in `r Each_State3$State[1]`.\
Followed is the Average Household Size map of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
HH.size.BG.ME <- tidycensus::get_acs(geography = "block group",
															survey="acs5",
															state=i,
															year = endyr,
															output = "wide",
...
tm_shape(HH.size.BG.ME) + tm_polygons("AveHouseH_BG", style = "quantile", palette="Purples", border.col = "darkgray", title="Ave Household Size") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite") + tmap_options(show.messages = FALSE)
...
Educ.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Educ.BG, by="BG_ID_10")
...
Each_State3 <- Educ.State
```
* The number of people, who have less than high school degree is 74426 in `r Each_State3$State[1]` which is `r (round(Each_State3$LessThanHSPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State3$State[1]`.\

* The number of people, who have high school degree is 308013 in `r Each_State3$State[1]` which is `r (round(Each_State3$HSGradPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State3$State[1]`.\

* The number of people, who have some college degree is 286179 in `r Each_State3$State[1]` which is `r (round(Each_State3$SomeCollPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State3$State[1]`.\

* The number of people, who have bachelor degree is 190172 in `r Each_State3$State[1]` which is `r (round(Each_State3$BachPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State3$State[1]`.\

* The number of people, who have master's degree is 78014 in `r Each_State3$State[1]` which is `r (round(Each_State3$MasterPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State3$State[1]`.\

* The number of people, who have professional school degree is 18281 in `r Each_State3$State[1]` which is `r (round(Each_State3$ProfPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State3$State[1]`.\

* The number of people, who have doctorate degree is 12812 in `r Each_State3$State[1]` which is `r (round(Each_State3$DocPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State3$State[1]`.\

Followed is the Educational Attainment map of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Educ.BG.ME <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
Single.HH.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Single.HH.BG, by="BG_ID_10")
...
Each_State3 <- Single.HH.State

```
* The percentage of single-female-headed is `r (round(Each_State3$FemHeadPer_State[1],2))*100`% of total number of family households in `r Each_State3$State[1]`.\
* The percentage of single-male-headed is `r (round(Each_State3$MaleHeadPer_State[1],2))*100`% of total number of family households in `r Each_State3$State[1]`.\
Followed is the Single Parent Households map of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Single.HH.BG.ME <- tidycensus::get_acs(geography = "block group",
															survey="acs5",
															state=i,
															year = endyr,
															output = "wide",
...
Poverty.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Poverty.BG, by="BG_ID_10")
...
Poverty.State <- tidycensus::get_acs(geography = "state",
  ...
  cbind(Poverty.BG) 
Each_State3 <- Poverty.State
```
* The percentage of family, who are in poverty is `r (round(Each_State3$FamPovPer_State[1],2))*100` % in `r Each_State3$State[1]`.\
* The percentage of population, who are in poverty is `r (round(Each_State3$PopPovPer_State[1],2))*100` % in `r Each_State3$State[1]`.\
* The total number of people, who are in poverty is 161743 in `r Each_State3$State[1]`.\
Followed is the Population in Poverty map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Poverty.BG.ME <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Poverty.BG.ME) + tm_polygons("FamPovPer_BG", style="jenks", palette="Purples", border.col = "darkgray", title="Family in Poverty",popup.format = list(fun = make_percent), legend.format = list(fun = make_percent)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
...
Each_State3 <- OldAlone.State %>%
  cbind(Each_State3)
## GIVE US NA
```
* The percentage of householders, who are living alone and are older than 65 years old is `r (round(Each_State3$OldAlonePer_State[1],2))*100` % in `r Each_State3$State[1]`.
...
* `r Each_State3$State[1]` has 276 public school district, which contain 599 public schools. Also, `r Each_State3$State[1]` has 118 private schools.(https://nces.ed.gov/)

\


```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State3$SchoolsDistrict <- 276
Each_State3$PubSchools <- 599
Each_State3$PvSchools <- 118
```
...
* `r Each_State3$State[1]` has 36 universities.(https://nces.ed.gov/)

```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State3$NumUni <- 36
```
...
Enrol.Sch.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Enrol.Sch.BG, by="BG_ID_10")
...
Each_State3 <- Enrol.Sch.State
```
* The total number of students, who enrolled in preschools to 12 grade levels in `r Each_State3$State[1]` schools is 206221.\
...
Num.jobs.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Num.jobs.BG, by="BG_ID_10")
...
Each_State3 <- Num.jobs.State
```
* The number of jobs within community boundaries in `r Each_State3$State[1]` is 623582.\

Followed is the map of Number of Jobs within Community Boundaries of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}

## MAP
Num.jobs.BG.ME <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Num.jobs.BG.ME) + tm_polygons("NumJobs_BG", style="jenks", palette="Reds", border.col = "darkgray", title="Number of Jobs") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
Place.Work.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Place.Work.BG, by="BG_ID_10")
...
Each_State3 <- Place.Work.State

```
* The number of people, who work in state of residence and worked in county of residence is 501291 in `r Each_State3$State[1]`, which is `r (round(Each_State3$WorkStateCountyPer_State[1],2))*100` % of total workers in `r Each_State3$State[1]`.\
\
...
Labor.Force.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Labor.Force.BG, by="BG_ID_10")
...
Each_State3 <- Labor.Force.State
```
* The number of labor force, who are 16 years and older in `r Each_State3$State[1]` is 699180 which is `r round(Each_State3$LabForcePer_State[1],2)` of total number of labors of which 697391 are civilian labor forces and 1789 are armed labor forces.\

Followed is the map of Labor Force of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Labor.Force.BG.ME <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Labor.Force.BG.ME) + tm_polygons("LabForce_BG", style="jenks", palette="seq", border.col = "darkgray", title="Number of Labors") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
Emp.Unemp.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Emp.Unemp.BG, by="BG_ID_10")
...
Each_State3 <- Emp.Unemp.State
```
* The rate of employment in `r Each_State3$State[1]` is `r round(Each_State3$EmpRate_State[1],2)*100`%.\
* The rate of unemployment in `r Each_State3$State[1]` is `r round(Each_State3$UnempRate_State[1],2)*100`%.\

Followed is Employed and Unemployed map of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Emp.Unemp.BG.ME <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Emp.Unemp.BG.ME) + tm_polygons(c("EmpRate_BG","UnempRate_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("Employment rate","Unemployment rate")) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
...
Emp.NCAIS.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Emp.NCAIS.BG, by="BG_ID_10")
...
Each_State3 <- Emp.NCAIS.State

```
* Number of male by industry for the civilian employed in `r Each_State3$State[1]` is 337904 which is `r round(Each_State3$MaleNCAISPer_State[1],2)` of population, who are 16 years and over.\
* Number of female by industry for the civilian employed in `r Each_State3$State[1]` is 327284 which is `r round(Each_State3$FemaleNCAISPer_State[1],2)` of population, who are 16 years and over.\

Followed is the map of the Major Employers by NCAIS Code of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Emp.NCAIS.BG.ME <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Emp.NCAIS.BG.ME) + tm_polygons(c("MaleNCAIS_BG","FemaleNCAIS_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("MaleNCAIS","FemaleNCAIS")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
...
Industries.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Industries.BG, by="BG_ID_10")
...
Each_State3 <- Industries.State
```
* The percentage of people, who are in agriculture, forestry, fishing, hunting, and mining in `r Each_State3$State[1]` is `r (round(Each_State3$Agriculture_State[1],2))*100`%.\

* The percentage of people, who are in construction in `r Each_State3$State[1]` state is `r (round(Each_State3$Construction_State[1],2))*100`%".\

* The percentage of people, who are in manufacturing in `r Each_State3$State[1]` state is `r (round(Each_State3$Manufacturing_State[1],2))*100`%.\

* The percentage of people, who are in wholesale trade in `r Each_State3$State[1]` state is `r (round(Each_State3$WholesaleTrade_State[1],2))*100`%.\

* The percentage of people, who are in retail trade in `r Each_State3$State[1]` state is `r (round(Each_State3$RetailTrade_State[1],2))*100`%.\

* The percentage of people, who are in transportation and warehousing, and utilities in`r Each_State3$State[1]`state is`r (round(Each_State3$Transportation_State[1],2))*100`%.\

* The percentage of people, who are in information in `r Each_State3$State[1]` state is `r (round(Each_State3$Information_State[1],2))*100`%.\

* The percentage of people, who are in finance and insurance, and real estate, and rental and leasing in `r Each_State3$State[1]` state is `r (round(Each_State3$Finance_Ins_State[1],2))*100`%.\

* The percentage of people, who are in professional, scientific, and management, and administrative, and waste management services in `r  Each_State3$State[1]` state is `r (round(Each_State3$Professional_State[1],2))*100`%.\

* The percentage of people, who are in educational services, and health care and social assistance in `r Each_State3$State[1]` state is `r (round(Each_State3$Educ_Health_SocSvcs_State[1],2))*100`%.\

* The percentage of people, who are in arts, entertainment, and recreation, and accommodation and food services in `r Each_State3$State[1]` state is `r (round(Each_State3$RecreationEtc_State[1],2))*100`%.\

* The percentage of people, who are in other services, except public administration in `r Each_State3$State[1]` state is `r (round(Each_State3$OtherIndustries_State[1],2))*100`%.\

* The percentage of people, who are in Public administration in `r Each_State3$State[1]` state is `r (round(Each_State3$PublicAdmin_State[1],2))*100`%.\
\
...
Med.Earning.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Med.Earning.BG, by="BG_ID_10")
...
Each_State3 <- Med.Earning.State
```
* The median earning in `r Each_State3$State[1]` is 31447 Dollar.\
Followed is the Median Earning map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
# Earning at Block Group level
Med.Earning.BG.ME <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Med.Earning.BG.ME) + tm_polygons("MedEarning_BG" , palette="seq", style = "jenks", title = "Median Earning", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") 
...
Emp.Age.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Emp.Age.BG, by="BG_ID_10")
...
Each_State3 <- Emp.Age.State
```
* The number of workers, who are younger than 29 years old in `r Each_State3$State[1]` is 146147 which is `r (round(Each_State3$AgeEmpU29Per_State[1],2))*100`% of total number of workers.\

* The number of workers, who are between 30 and 54 years old in `r Each_State3$State[1]` is 336245 which is `r (round(Each_State3$AgeEmp3054Per_State[1],2))*100`% of total number of workers.\

* The number of workers, who are older than 55 years old in `r Each_State3$State[1]` is 184585 which is `r (round(Each_State3$AgeEmpO55Per_State[1],2))*100`% of total number of workers.\
...
Emp.Race.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Emp.Race.BG, by="BG_ID_10")
...
Each_State3 <- Emp.Race.State 
```

* The number of white workers in `r Each_State3$State[1]` is 636364 which is `r (round(Each_State3$WhiteEmpPer_State[1],2))*100`% of total number of workers.\

* The number of black workers in `r Each_State3$State[1]` is 16990 which is `r (round(Each_State3$BlackEmpPer_State[1],2))*100`% of total number of workers.\

* The number of asian workers in `r Each_State3$State[1]` is 7915 which is `r (round(Each_State3$AsianEmpPer_State[1],2))*100`% of total number of workers.\

* The number of hispanic or latino workers in `r Each_State3$State[1]` is 10382 which is `r (round(Each_State3$HispanicEmpPer_State[1],2))*100`% of total number of workers.\

* The number of workers, who have two or more races in `r Each_State3$State[1]` is 10145 which is `r (round(Each_State3$TwoOrMoreEmpPer_State[1],2))*100`% of total number of workers.\
\
...
Emp.Educ.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Emp.Educ.BG, by="BG_ID_10")
...
Each_State3 <- Emp.Educ.State 
```
* The number of workers, who have less than high school degree in `r Each_State3$State[1]` is 23834 which is `r (round(Each_State3$LessThanHSLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have high school degree in `r Each_State3$State[1]` is 172523 which is `r (round(Each_State3$HSGradLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have some college or associates degree in `r Each_State3$State[1]` is 193504 which is `r (round(Each_State3$SomeCollLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have bachelors degree or higher in `r Each_State3$State[1]` is 215585 which is `r (round(Each_State3$BachOrHigherLaborPer[1],2))*100`% of total number of workers.\
...
Emp.Sex.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Emp.Sex.BG, by="BG_ID_10")
...
Each_State3 <- Emp.Sex.State

```
* The number of male workers in `r Each_State3$State[1]` is 339548 which is `r (round(Each_State3$EmpMalePer[1],2))*100`% of total number of workers.\
* The number of female workers in `r Each_State3$State[1]` is 327429 which is `r (round(Each_State3$EmpFemalePer[1],2))*100`% of total number of workers.\
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State3$TotalRevenueFY19 <- 3668205427
Each_State3$REVSalesUseTaxFY19 <- 1467441307
Each_State3$REVServiceProviderTaxFY19 <- 62454424
Each_State3$REVIndividualIncomeTaxFY19 <- 1619358151
Each_State3$REVCorporateIncomeTaxFY19 <- 204329196
Each_State3$REVCigaretteTaxFY19 <- 134099350
Each_State3$REVInsuranceTaxFY19 <- 74150000
Each_State3$REVInheritanceEstateTaxFY19 <- 12640409
Each_State3$REVFineForfeituresFY19 <- 18205011
Each_State3$REVIncomeInvestmentsFY19 <- 5831119
Each_State3$REVLotteryFY19 <- 57000000
Each_State3$REVTransTaxReliefProgsFY19 <- 66388623
Each_State3$REVTransMuniShareFY19 <- 69244574
Each_State3$REVOtherTaxesFY19 <- 139776209
Each_State3$REVOtherRevenueFY19 <- 10111697
```
* The total revenues for `r Each_State3$State[1]` is $ 3,668,205,427 in Fiscal Year 2019, which includes sales and use tax with 1,467,441,307 Dollar, service provider tax with 62,454,424, individual income tax with 1,619,358,151 Dollar, corporate income tax with 204,329,196 Dollar, cigarette & tobacco tax with 134,099,350 Dollar, insurance company tax with 74,150,000 Dollar, inheritance & estate tax with 12,640,409 Dollar, fines, forfeits and penalties with 18,205,011 Dollar, income from investments with 5,831,119 Dollar, transfer from lottery with 57,000,000 Dollar, transfer for tax relief programs with 66,388,623 Dollar, transfer to municipal revenue share with 70,802,823 Dollar, other taxes and fees with 139,776,209 Dollar, and other revenues with 10,111,697 Dollar. (https://www.maine.gov/budget/)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State3$ExpendituresFY19 <- 3700598942
Each_State3$EXEducationFY19 <- 1639144066
Each_State3$EXEconDevelopmenFY19 <- 43591773
Each_State3$EXGeneralGovernmentFY19 <- 332306853
Each_State3$EXHumanServicesFY19 <- 1248885662
Each_State3$EXNaturalResourcesFY19 <- 80761979
Each_State3$EXProtectionFY19 <- 347689953
Each_State3$EXCulturalFY19 <- 8218656
```

* The total amount of expenditures for `r Each_State3$State[1]` is $3,700,598,942 in Fiscal Year 2019 and the major expense driver is related to education with 1,639,144,066 Dollar. Also, expenditures include governmental support & operations with 332,306,853 Dollar, economic development & work force training with 43,591,773 Dollar, health & human services with 1,248,885,662 Dollar, natural resources development & protection with 80,761,979 Dollar, justice & protection with 347,689,953 Dollar, and  arts, heritage & cultural enrichment with 8,218,656 Dollar.(https://www.maine.gov/budget/)
...
Snap.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Snap.BG, by="BG_ID_10")
...
Each_State3 <- Snap.State 

#######################################################################################################################################

```
* The percent of households, who received Food Stamps/SNAP in the past 12 months in `r Each_State3$State[1]` state is `r (round(Each_State3$SnapRec_State[1],2))*100`%.\
...
Housing.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Housing.BG, by="BG_ID_10")
...
Each_State3 <- Housing.State
```
* The median house value in `r Each_State3$State[1]` is 184500 Dollar.\

* The average house value in `r Each_State3$State[1]` is 226322 Dollar.\

* The proportion of renters to owners in `r Each_State3$State[1]` is `r round(Each_State3$RentersToOwners_State[1],2)`.\

* The percentage of renters in `r Each_State3$State[1]` is `r (round(Each_State3$RentersPer_State[1],2))*100`%.\

* The percentage of owners in `r Each_State3$State[1]` is `r (round(Each_State3$HomeOwnPer_State[1],2))*100`%.\

Followed is the map of Proportion of Renters to Owners, Median Home Values, and Average Home Values of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Housing.BG.ME <- tidycensus::get_acs(geography = "block group",
															 survey="acs5",
															 state=i,
															 year = endyr,
															 output = "wide",
...
tm_shape(Housing.BG.ME) + tm_polygons("MedianHValue_BG" , palette="Blues", style = "quantile", title = "Median Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("right","bottom"))


tm_shape(Housing.BG.ME) + tm_polygons("AvgHValue_BG" , palette="Greens", style = "quantile", title = "Average Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray")+tm_layout(legend.position = c("right","bottom"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(Housing.BG.ME) + tm_polygons("RentersToOwners_BG" , palette="Reds", style = "jenks", title = "RentersToOwners") + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("right","bottom"))
```
...
Dur.Occ.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, Dur.Occ.BG, by="BG_ID_10")
...
Each_State3 <- Dur.Occ.State

```
* The median year that a homeowner moved into their current domicile in `r Each_State3$State[1]` is `r Each_State3$MedYrOwnMovedIn_State[1]`.\
* The median year that a renter moved into their current domicile in `r Each_State3$State[1]` is `r Each_State3$MedYrRentMovedIn_State[1]`.\
Followed is the map of Duration of Occupancy of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Dur.Occ.BG.ME <- tidycensus::get_acs(geography = "block group",
															 survey="acs5",
															 state=i,
															 year = endyr,
															 output = "wide",
...
tm_shape(Dur.Occ.BG.ME) + tm_polygons(c("MedYrMovedIn_BG","MedYrRentMovedIn_BG","MedYrOwnMovedIn_BG"), style="jenks", palette=list("Greens","seq","Blues"), border.col = "darkgray", title=c("MedYrMovedIn","MedYrRentMovedIn","MedYrOwnMovedIn")) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
CostBurdened.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State3, CostBurdened.BG, by="BG_ID_10")
...
MA.Vars$Indicators[MA.Vars$Indicators == 'REVServiceProviderTaxFY19'] <- "RevenueServiceProviderTaxFY19"
MA.Vars$Indicators[MA.Vars$Indicators == 'REVIndividualIncomeTaxFY19'] <- 'RevenueIndividualIncomeTaxFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVCorporateIncomeTaxFY19'] <- 'RevenueCorporateIncomeTaxFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVCigaretteTaxFY19'] <- 'RevenueCigaretteTaxFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVInsuranceTaxFY19'] <- 'RevenueInsuranceTaxFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVInheritanceEstateTaxFY19'] <- 'RevenueInheritanceEstateTaxFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVFineForfeituresFY19'] <- 'RevenueFineForfeituresFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVIncomeInvestmentsFY19'] <- 'RevenueIncomeInvestmentsFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVTransTaxReliefProgsFY19'] <- 'RevenueTransTaxReliefProgsFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVLotteryFY19'] <- 'RevenueLotteryFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVTransMuniShareFY19'] <- 'RevenueTransMuniShareFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVOtherTaxesFY19'] <- 'RevenueOtherTaxesFY19FY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'REVOtherRevenueFY19'] <- 'RevenueOtherRevenueFY19'
...
MA.Vars$Indicators[MA.Vars$Indicators == 'EXEconDevelopmenFY19'] <- 'ExpendituresEconDevelopmenFY19'
...
MA.Vars$Indicators[MA.Vars$Indicators == 'EXNaturalResourcesFY19'] <- 'ExpendituresNaturalResourcesFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'EXProtectionFY19'] <- 'ExpendituresProtectionFY19'
MA.Vars$Indicators[MA.Vars$Indicators == 'EXCulturalFY19'] <- 'ExpendituresCulturalFY19'
...
StateCities
write.csv(StateCities,"~/Desktop/MaineCities.csv", row.names = FALSE)
```
