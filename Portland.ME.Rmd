---
title: "Municipal Indicators For Portland, ME"
output:
  pdf_document:
    toc: yes
    toc_depth: '3'
...
In this report, 40 demographic, educational institutions, employment, healthcare, municipal finance, infrastructure, food insecurity and housing variables are presented using federally-published data sources for the city of Portland, Maine. To better demonstrate their ties to geography of the city, the variables have been mapped on the block group level. \
...
```
* The median household income in the past 12 months is 56977 Dollar in `r Chelsea.MA$City`.\
...
* `r Chelsea.MA$City` has 5 public school district, which contains 10596 students and 26 public schools. Also, `r  Chelsea.MA$City` has 8 private schools with 1838 students.(https://nces.ed.gov/) 

\


```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$SchoolsDistrict <- 5
Chelsea.MA$SchoolsDistStudents <- 10596
Chelsea.MA$PubSchools <- 26
Chelsea.MA$PvSchools <- 8
```
...
* `r Chelsea.MA$City` has 5 university in 04101, 04102, 04103, and 04106 ZIP Codes.(https://nces.ed.gov/) 
...
* The largest School District in `r Chelsea.MA$City` contains 587 teachers and 616 other staffs. The total number of teachers in all private schools in `r Chelsea.MA$City` is 261.(http://www.usa.com/)
...
```
* The number of labor force, who are 16 years and older in `r Chelsea.MA$City` is 41035 which is `r round(Chelsea.MA$LabForcePer,2)` of total number of labors (unemployed and not in labor force) of which 40942 are civilian labor forces and `r Chelsea.MA$Military` are armed labor forces.\
...
```
* The median earning in `r Chelsea.MA$City` is 34041 Dollar.\
...
* `r Chelsea.MA$City` has 4 hospitals.(https://www.ahd.com/)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$NursingHomes <- 4
```


* `r Chelsea.MA$City` has 4 nursing homes. (https://nursinghomedirectory.com/)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$RevenueGeneralFundTotalFY19 <- 194461142
Chelsea.MA$RevenuePropertyAndExciseTaxesFY19 <- 99711603
Chelsea.MA$RevenueLicensesPermitsFY19 <- 5827244
Chelsea.MA$RevenueIntergovernmentalFY19 <- 10464979
Chelsea.MA$RevenueChargesForServicesFY19 <- 35867048
Chelsea.MA$RevenueFinesForfeitsPenaltiesFY19 <- 2107635
Chelsea.MA$RevenueUsesOfMoneyAndPropertyFY19 <- 11175951
Chelsea.MA$RevenueOtherFinancingSourcesFY19 <- 29306682
```
* The total budget revenues for general fund for Portland city is $194,461,142 in fiscal year 2019, which contains the following items: Property and excise taxes with 99,711,603 Dollar, licenses and permits with 5,827,244 Dollar, intergovernmental with 10,464,979 Dollar, charges for services with 35,867,048 Dollar, fines, forfeits, and penalties with 2,107,635 Dollar, uses of money and property with 11,175,951 Dollar, and other financing resources with 29,306,682 Dollar.(http://www.portlandmaine.gov/ArchiveCenter/ViewFile/Item/929)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$ExpendituresGeneralFundTotalFY19 <- 194461142
Chelsea.MA$ExpendituresGeneralGovernmentFY19 <- 48237542
Chelsea.MA$ExpendituresPublicSafetyFY19 <- 34157018
Chelsea.MA$ExpendituresPublicWorksFY19 <- 13971411
Chelsea.MA$ExpendituresParksRecreationFacilitiesFY19 <- 11194057
Chelsea.MA$ExpendituresPublicBuildingsWaterfrontFY19 <- 5019804
Chelsea.MA$ExpendituresHealthAndHumanServicesFY19 <- 26998780
Chelsea.MA$ExpendituresLibraryFY19 <- 4062000
Chelsea.MA$ExpendituresCountyTaxFY19 <- 6288845
Chelsea.MA$ExpendituresMetroFY19 <- 2713649
Chelsea.MA$ExpendituresLongTermDebtFY19 <- 41818036
```

* The total amount of general fund of expenditures for `r Chelsea.MA$City` is $194,461,142 in Fiscal Year 2019 and the major expense driver is related to general government, which is 48,237,542 Dollar. The other expenditures include public safety 34,157,018 Dollar, public works with 13,971,411 Dollar, parks, recreation and facilities with 11,194,057 Dollar, public buildings waterfront with 5,019,804 Dollar, health and human services with 26,998,780 Dollar, library with 4,062,000 Dollar, conuty tax with 6,288,845 Dollar, metro with 2,713,649 Dollar, and long term debt with 41,818,036 Dollar.(http://www.portlandmaine.gov/ArchiveCenter/ViewFile/Item/929)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$RevenueGeneralFundTotalFY20 <- 206603707
Chelsea.MA$RevenuePropertyTaxesFY20 <- 93184789
Chelsea.MA$RevenueOtherLocalTaxesFY20 <- 8416388
Chelsea.MA$RevenueLicensesPermitsFeesFY20 <- 6300644
Chelsea.MA$RevenueIntergovernmentalFY20 <- 12312490
Chelsea.MA$RevenueChargesForServicesFY20 <- 36750588
Chelsea.MA$RevenueFinesForfeitsPenaltiesFY20 <- 1996710
Chelsea.MA$RevenueUseOfMoneyAndPropertyFY20 <- 11779430
Chelsea.MA$RevenueOtherSourcesFY20 <- 35862668

Chelsea.MA$ExpendituresGeneralFundTotalFY20 <- 197175619
Chelsea.MA$ExpendituresCityCouncilFY20 <- 344046
Chelsea.MA$ExpendituresCityClerkFY20 <- 660721
Chelsea.MA$ExpendituresExecutiveFY20 <- 929877
Chelsea.MA$ExpendituresAssessorFY20 <- 435159
Chelsea.MA$ExpendituresTotalFinanceFY20 <- 1952730
Chelsea.MA$ExpendituresLegalFY20 <- 741338
Chelsea.MA$ExpendituresHumanResourcesFY20 <- 1071134
Chelsea.MA$ExpendituresTotalParkingFY20 <- 2366214
Chelsea.MA$ExpendituresEconomicDevelopmentFY20 <- 674528
Chelsea.MA$ExpendituresPoliceFY20 <- 17757540
Chelsea.MA$ExpendituresFireFY20 <- 18235148
Chelsea.MA$ExpendituresPlanningAndUrbanDevelopmentFY20 <- 2080032 
Chelsea.MA$ExpendituresPermittingAndLicensingFY20 <- 1848559
Chelsea.MA$ExpendituresInformationTechnologyFY20 <- 2914482
Chelsea.MA$ExpendituresPublicWorksFY20 <- 14636202
Chelsea.MA$ExpendituresParksRecreationFacilitiesFY20 <- 11457902
Chelsea.MA$ExpendituresPublicBuildingsAndWaterfrontFY20 <- 5080341
Chelsea.MA$ExpendituresHHSFY20 <- 31451520
Chelsea.MA$ExpendituresDebtServiceFY20 <- 45087798
Chelsea.MA$ExpendituresPublicLibraryFY20 <- 4178550
Chelsea.MA$ExpendituresPernsionFY20 <- 8398292
Chelsea.MA$ExpendituresEmployeeBenefitsFY20 <- 23433313
Chelsea.MA$ExpendituresContingentFY20 <- 250000
Chelsea.MA$ExpendituresLiabilityInsuranceFY20 <- 839325
Chelsea.MA$ExpendituresRegionalTransportationProgramMembershipsFY20 <- 515868
Chelsea.MA$ExpendituresWageAdjustmentFY20 <- 165000
Chelsea.MA$ExpendituresCountyTaxFY20 <- 6640316
Chelsea.MA$ExpendituresMetroAssessmentFY20 <- 2787772
Chelsea.MA$ExpendituresTotalGeneralFundAndAssessmentFY20 <- 206603707
```

* The total general fund revenues for Portland city is 206,603,707 Dollar in fiscal year 2020, which contains the following items: Property tax revenue with 93,184,789 Dollar, other local taxes with 8,416,388 Dollar, licenses and permits with 6,300,644 Dollar, intergovernmental revenue with 12,312,490 Dollar, charges for services with 36,750,588 Dollar, fines, forfeits, and penalties with 1,996,710 Dollar, use of money and property with 11,779,430 Dollar, and other sources with 35,862,668 Dollar. The total general fund expenditures for `r Chelsea.MA$City` is $197,175,619 in Fiscal Year 2020 and the total general fund and assessments is 206,603,707 Dollar. The expenditures include city council with 344,046 Dollar, city clerk with 660,721 Dollar, excutive with 929,877 Dollar, assessor with 435,159 Dollar, finance with 1,952,730 Dollar, legal with 741,338 Dollar, human resources with 1,071,134 Dollar, parking/garages with 2,366,214 dollar, economic development with 674,528 Dollar, police with 17,757,540 Dollar, fire with 18,235,148 Dollar, planning and urban development with 2,080,032 Dollar, permitting and licensing with 1,848,559 Dollar, information technology with 2,914,482 Dollar, public works with 14,636,202 Dollar, parks recreation and facilities with 11,457,902 Dollar, public buildings and waterfront with 5,080,341 Dollar, HHS-administration with 31,451,520 Dollar, debt service with 45,087,798 Dollar, public library with 4,178,550 dollar, pension with 8,398,292 Dollar, employee benefits with 23,433,313, contingent with 250,000 Dollar, liability insurance with 839,325 Dollar, regional transportation program with 515,868 Dollar, wage adjustment with 165,000 Dollar, county tax with 6,640,316 Dollar, and metro assessment with 2,787,772 Dollar.(http://www.portlandmaine.gov/DocumentCenter/View/16673/City-Managers-Recommended-FY20-Budget-Blue-Book?bidId=)
...
* The Port of Portland is the largest tonnage seaport in New England as well as a major seaport along the east coast of the United States. Portland contains an international airport, whose name is Portland International Jetport. It is a public airport, which is located two miles west of downtown Portland, in Cumberland County, Maine.(https://en.wikipedia.org/)
...
* The METRO Breez provides daily express bus service between the Portland Transportation Center/ Downeaster Station and Falmouth, Yarmouth, Freeport, and  Brunswick. Also, there are other transportation options in Portland, which are GOMaine Ride Share and it provides services in Cumberland and York County with sharing a ride. The other option is Amtrak Downeaster, which provides car-free connection from Brunswick, Maine to Boston, Massachusetts and points in between. The other option is Utilize a TNC, which provides services such as Uber, Lyft or taxi.(https://www.portlandmaine.gov/)
...
* Portland includes 8 Internet service providers marketing home service, and 25 Internet service providers including specialized business and mobile providers such as OTELCO and TPx Communications. The most widely available provider in Portland is  Viasat Internet, which about one hundred percent of Portland census blocks can get Internet service from them. HughesNet is the most likely second choice in the area, serving nearly one hundred percent of Portland with Satellite service. Portland is the 21st most connected city in Maine and the fastest zip code in Portland for May 2020 is 04103. (https://broadbandnow.com/Maine/Portland)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$NumGrocery <- 72
```
* `r Chelsea.MA$City` has 72 Grocery Stores. (https://www.city-data.com/city/Portland-Maine.html)
...
```
* The median house value in `r Chelsea.MA$City` is 273400 Dollar.\

* The average house value in `r Chelsea.MA$City` is 4281016400 Dollar.\
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$PropertyTaxRate <- 23.31
```
* The 2020 tax rate is 23.31 Dollar per 1000 Dollar of assessed valuation in `r Chelsea.MA$City`.(https://www.portlandmaine.gov/135/Assessor)
...
tm_shape(chelsea.dur) + tm_polygons(c("MedYrMovedIn_BG","MedYrRentMovedIn_BG","MedYrOwnMovedIn_BG"), style="quantile", palette=list("Greens","seq","Blues"), border.col = "darkgray", title=c("MedYrMovedIn","MedYrRentMovedIn","MedYrOwnMovedIn")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
```
...
```{r}
library(data.table)
portland <- as.data.frame(t(as.matrix(portland)))
colnames(portland) <- portland[1,]
setDT(portland, keep.rownames = TRUE)[]
names(portland)[1] <- 'Indicators'
portland
```

```{r}
library(data.table)
millinocket <- as.data.frame(t(as.matrix(millinocket)))
colnames(millinocket) <- millinocket[1,]
setDT(millinocket, keep.rownames = TRUE)[]
names(millinocket)[1] <- 'Indicators'
millinocket
```
```{r}
Belfast.ME <- as.data.frame(t(as.matrix(Belfast.ME)))
colnames(Belfast.ME) <- Belfast.ME[1,]
setDT(Belfast.ME, keep.rownames = TRUE)[]
names(Belfast.ME)[1] <- 'Indicators'
Belfast.ME
```

```{r}
two.cities <- merge(portland, millinocket, by="Indicators", all=TRUE)
two.cities
```

```{r}
Cities.MA <- merge(two.cities,Belfast.ME, by="Indicators", all=TRUE)
Cities.MA
```
...
Cities.MA
write.csv(Cities.MA,"~/Desktop/Cities.ME.csv", row.names = FALSE)
```
