---
title: "Municipal Indicators For Newport, RI"
output:
  pdf_document:
    toc: yes
    toc_depth: '3'
...
In this report, 40 demographic, educational institutions, employment, healthcare, municipal finance, infrastructure, food insecurity and housing variables are presented using federally-published data sources for the city of Newport, Rhode Island. To better demonstrate their ties to geography of the city, the variables have been mapped on the block group level. \
...
```
* The median household income in the past 12 months is 65431 Dollar in `r Chelsea.MA$City`.\
...
tm_shape(chelsea.HH.Size) + tm_polygons("AveHouseH", style = "quantile", palette="Purples", border.col = "darkgray", title="Ave Household Size") + tm_view(alpha = 0.85,view.legend.position = c("right", "top"), basemaps = "Stamen.TonerLite") + tmap_options(show.messages = FALSE)
...
tm_shape(chelsea.pov) + tm_polygons("FamPovPer_BG", style="quantile", palette="Purples", border.col = "darkgray", title="Family in Poverty", popup.format = list(fun = make_percent), legend.format = list(fun = make_percent))  + tm_view(alpha = 0.85,view.legend.position = c("right", "top"), basemaps = "Stamen.TonerLite")
...
* `r Chelsea.MA$City` has 1 public school district, which contains 2156 students and 4 public schools. Also, `r  Chelsea.MA$City` has 1 private schools with 199 students.(https://nces.ed.gov/) 
...
Chelsea.MA$SchoolsDistStudents <- 2156
...
* `r Chelsea.MA$City` has 2 university in 02840 ZIP Code.(https://nces.ed.gov/) 
...
* The largest School District in `r Chelsea.MA$City` contains 129 teachers and 91 other staffs. The total number of teachers in a private school in `r Chelsea.MA$City` is 38.(http://www.usa.com/)
...
```
* The number of labor force, who are 16 years and older in `r Chelsea.MA$City` is 15207 which is `r round(Chelsea.MA$LabForcePer,2)` of total number of labors (unemployed and not in labor force) of which 12919 are civilian labor forces and `r Chelsea.MA$Military` are armed labor forces.\
...
```
* The median earning in `r Chelsea.MA$City` is 33825 Dollar.\
...
* `r Chelsea.MA$City` has 1 hospital with 129 beds.(https://www.ahd.com/)
...
* `r Chelsea.MA$City` has 3 nursing homes. (https://nursinghomedirectory.com/)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$RevenueGeneralFundTotalFY19 <- 97784318
Chelsea.MA$RevenueLocalTaxesFY19 <- 82256525
Chelsea.MA$RevenueStateAndFederalAidFY19 <- 3771286
Chelsea.MA$RevenueServiceChargesFeesFY19 <- 9978223
Chelsea.MA$RevenueUsesOfMoneyAndPropertyFY19 <- 940129
Chelsea.MA$RevenueContributionsAndSaleOfPropertyFY19 <- 260002
Chelsea.MA$RevenueOtherSourcesFY19 <- 578153
```
* The total general fund revenues for Newport city is $97,784,318 in fiscal year 2019, which contains following items: Local taxes with 82,256,525 Dollar, state and federal aid with 3,771,286 Dollar, charges for services with 9,978,223 Dollar, use of money and property with 940,129 Dollar, contributions and sale of property with 260,002 Dollar, and other sources with 578,153 Dollar.(https://www.cityofnewport.com/CityOfNewport/media/City-Hall/Departments/Finance/Budgets/FY2021-Proposed-Final-Combined.pdf)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$ExpendituresGeneralFundTotalFY19 <- 98183639
Chelsea.MA$ExpendituresFiduciaryAccountsFY19 <- 30431898
Chelsea.MA$ExpendituresDebtServiceFY19 <- 5507202
Chelsea.MA$ExpendituresReserveAccountsFY19 <- 905169
Chelsea.MA$ExpendituresCityManagerFY19 <- 939472
Chelsea.MA$ExpendituresFinanceDepartmentFY19 <- 4299660
Chelsea.MA$ExpendituresPoliceFY19 <- 18420518
Chelsea.MA$ExpendituresFireFY19 <- 19759948
Chelsea.MA$ExpendituresPublicServicesFY19 <- 11202111
Chelsea.MA$ExpendituresDeptOfPlanningFY19 <- 167278
Chelsea.MA$ExpendituresDeptOfZoningAndInspectionsFY19 <- 1091552
Chelsea.MA$ExpendituresTransfersFY19 <- 3975979
```

* The total amount of general fund expenditures for `r Chelsea.MA$City` is $98,183,639 in Fiscal Year 2019 and the major expense driver is related to fiduciary accounts, which is 30,431,898 Dollar. The other expenditures include debt service with 5,507,202 Dollar, reserve accounts with 905,169 Dollar, city manager with 939,472 Dollar, finance departmen with 4,299,660 Dollar, police with 18,420,518 Dollar, fire with 19,759,948 Dollar, public services with 11,202,111 Dollar, dept of planning with 167,278 Dollar, dept of zoning and inspections with 1,091,552 Dollar, and subtotal transfers with 3,975,979 Dollar.(https://www.cityofnewport.com/CityOfNewport/media/City-Hall/Departments/Finance/Budgets/FY2021-Proposed-Final-Combined.pdf)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$RevenueGeneralFundTotalFY20 <- 99981982
Chelsea.MA$RevenueLocalTaxesFY20 <- 84917756
Chelsea.MA$RevenueStateAndFederalAidFY20 <- 3885706
Chelsea.MA$RevenueServiceChargesFeesFY20 <- 10517020
Chelsea.MA$RevenueUsesOfMoneyAndPropertyFY20 <- 490000
Chelsea.MA$RevenueContributionsAndSaleOfPropertyFY20 <- 171500


Chelsea.MA$ExpendituresGeneralFundTotalFY20 <- 99981982
Chelsea.MA$ExpendituresFiduciaryAccountsFY20 <- 30667738
Chelsea.MA$ExpendituresDebtServiceFY20 <- 5424913
Chelsea.MA$ExpendituresReserveAccountsFY20 <- 934300
Chelsea.MA$ExpendituresCityManagerFY20 <- 1117050
Chelsea.MA$ExpendituresFinanceDepartmentFY20 <- 4323365
Chelsea.MA$ExpendituresPoliceFY20 <- 19012577
Chelsea.MA$ExpendituresFireFY20 <- 20050254
Chelsea.MA$ExpendituresPublicServicesFY20 <- 11262846
Chelsea.MA$ExpendituresDeptOfPlanningFY20 <- 509477
Chelsea.MA$ExpendituresDeptOfZoningAndInspectionsFY20 <- 959979
Chelsea.MA$ExpendituresTransfersFY20 <- 4039286
```

* The total adopted budget for general fund revenues for Newport city is 99,981,982 Dollar in fiscal year 2020, which contains following items: Local taxes with 84,917,756 Dollar, state and federal aid with 3,885,706 Dollar, charges for services with 10,517,020 Dollar, use of money and property with 490,000 Dollar, and contributions and sale of property with 171,500 Dollar. The total amount of adopted budget for general fund expenditures for `r Chelsea.MA$City` is $99,981,982 in Fiscal Year 2020 and the major expense driver is related to fiduciary accounts, which is 30,667,738 Dollar. The other expenditures include debt service with 5,424,913 Dollar, reserve accounts with 934,300 Dollar, city manager with 1,117,050 Dollar, finance departmen with 4,323,365 Dollar, police with 19,012,577 Dollar, fire with 20,050,254 Dollar, public services with 11,262,846 Dollar, dept of planning with 509,477 Dollar, dept of zoning and inspections with 959,979 Dollar, and subtotal transfers with 4,039,286 Dollar.(https://www.cityofnewport.com/CityOfNewport/media/City-Hall/Departments/Finance/Budgets/FY2021-Proposed-Final-Combined.pdf)
...
* There is regional airport (Newport State Airport) in Newport city that servicing domestic and international airlines in addition regional lines and private aircraft. Newport State Airport is located two nautical miles (4 km) northeast of its central business district.(https://www.discovernewport.org/listing/newport-airport/). The Newport & Narragansett Bay Railroad offers a classic railroad travel. The train's route takes passengers to the west side of Aquidneck Island. (http://trainsri.com/about/)
...
* RIPTA (Rhode Island Public Transit Authority), based at the Newport Gateway Center, operates buses and trolley-buses. It includes a Bus which going along Bellevue Avenue to the Newport mansions, and another bus that connecting downtown Newport with Middletown and its motels. Also, there are Newport Harbor Shuttle for residents in the Newport city. (https://newenglandtravelplanner.com/go/ri/newport/trans/transport_in_newport.html)
...
* Cable, DSL Internet service, and Verizon High Speed Internet are the main broadband network types with availability around Newport city. Viasat Internet and HughesNet are the most common Internet options in Newport. The average home still has 2 companies able to service the address. As it stands, only 1,000 Newport residents have access to one or fewer providers. There are 11 internet providers in Newport with 4 of those offering residential service. Newport is the 55th most connected city in Rhode Island and the fastest zip code in Newport for June 2020 is 02840.(https://broadbandnow.com/Rhode-Island/Newport)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$NumGrocery <- 12
```
* `r Chelsea.MA$City` has 12 Grocery Stores.(https://www.city-data.com/city/Newport-Rhode-Island.html)
...
```
* The median house value in `r Chelsea.MA$City` is 425400 Dollar.\

* The average house value in `r Chelsea.MA$City` is 2624010600 Dollar.\
...
tm_shape(chelsea.housing) + tm_polygons("MedianHValue_BG" , palette="Blues", style = "quantile", title = "Median Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("right","top"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(chelsea.housing) + tm_polygons("AvgHValue_BG" , palette="Greens", style = "quantile", title = "Average Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray")+tm_layout(legend.position = c("right","top"))
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(chelsea.housing) + tm_polygons("RentersToOwners_BG" , palette="Reds", style = "jenks", title = "RentersToOwners") + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("right","top"))
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Chelsea.MA$PropertyTaxRateResidential <- 10.28
Chelsea.MA$PropertyTaxRateCommercial <- 15.42
```
* The 2019-2020 tax rate for residential properties is 10.28 Dollar per 1000 Dollar of assessed valuation in `r Chelsea.MA$City` and for commercial properties is $15.42 per thousand.(https://www.cityofnewport.com/en-us/city-hall/departments/finance)
...
tm_shape(chelsea.dur) + tm_polygons(c("MedYrMovedIn_BG","MedYrRentMovedIn_BG","MedYrOwnMovedIn_BG"), style="jenks", palette=list("Greens","seq","Blues"), border.col = "darkgray", title=c("MedYrMovedIn","MedYrRentMovedIn","MedYrOwnMovedIn")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","top"))
```
...
* There are some assistance programs in Newport County such as; rental assistance, free food, or foreclosure counseling. The resources and services offered include the following items; energy and heating bills, energy efficiency program, energy audit which gives the applicants education on electric conservation techniques, and rent payment assistance that offers services to qualified individuals who can get eviction prevention funding and financial assistance. https://www.needhelppayingbills.com/html/newport_county_assistance_prog.html)
