---
title: "Indicators For Connecticut"
output:
  pdf_document:
    toc: yes
    toc_depth: '3'
...
In this report, 30 demographic, educational institutions, employment, municipal finances, food insecurity and housing variables are presented using federally-published data sources for Connecticut state. To better demonstrate their ties to geography of the state, the variables have been mapped on the block group level. \
...
Each_State4 <- Pop.State
```


The total population of `r Each_State4$State[1]` is 3581504. Followed is the popuation size map of the state.\
\

```{r , echo=FALSE, warning=FALSE, message=FALSE, comment=""}
## MAP
Pop.BG.CT <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Pop.BG.CT) + tm_polygons('TotalPop_BG' , palette="seq", style = "jenks", title = "Total Population")  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray")
```
\
...
Age.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, by="BG_ID_10") %>%
...
Each_State4 <- Age.State
```
* The number of people, who are younger than 18 Years old is 752625 in `r Each_State4$State[1]` 
, which is `r (round(Each_State4$AgeU18Per_State[1],2))*100`% of total population in `r Each_State4$State[1]`. \
* The number of people, who are between 18 and 34 years old is 790019 in `r Each_State4$State[1]`, which is `r (round(Each_State4$Age1834Per_State[1],2))*100`% of total population in `r Each_State4$State[1]`.\
* The number of people, who are between 35 and 64 years old is 1451280 in `r Each_State4$State[1]`, which is `r (round(Each_State4$Age3564Per_State[1],2))*100`% of total population in `r Each_State4$State[1]`.\
* The number of people, who are older than 65 years old is 587580 in `r Each_State4$State[1]`, which is `r (round(Each_State4$AgeO65Per_State[1],2))*100`% of total population in `r Each_State4$State[1]`.\

Followed is the popuation age map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## Map
Age.BG.CT <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
tm_shape(Age.BG.CT) + tm_polygons(c('AgeU18Per_BG','Age1834Per_BG',"Age3564Per_BG","AgeO65Per_BG") , palette=list("Oranges", "Greens", "Blues", "Purples") ,style = "quantile", title = c('AgeU18','Age1834',"Age3564","AgeO65"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray") + tmap_options(show.messages = FALSE) + tm_layout(legend.position = c("right","bottom"))
...
Each_State4 <- Income.State
```
* The median household income in the past 12 months is 76106 Dollar in `r Each_State4$State[1]`.\
Followed is the Median Household Income map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Income.BG.CT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
Race.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Race.BG, by="BG_ID_10") %>%
...
Each_State4 <- Race.State
```
* The number of people who are white in `r Each_State4$State[1]` is 2418696 which is `r (round(Each_State4$WhitePer_State[1],2))*100`% of total population in `r Each_State4$State[1]`.\
* The number of people who are black in `r Each_State4$State[1]` is 351817 which is `r (round(Each_State4$BlackPer_State[1],2))*100`% of total population in `r Each_State4$State[1]`.\
* The number of people who are asian in `r Each_State4$State[1]` is 157406 which is `r (round(Each_State4$AsianPer_State[1],2))*100`% of total population in `r Each_State4$State[1]`.\
* The number of people who are hispanic in `r Each_State4$State[1]` is 561791 which is `r (round(Each_State4$HispanicPer_State[1],2))*100`% of total population in `r Each_State4$State[1]`.\
* The number of people who are have two or more races in `r Each_State4$State[1]` is 74301 which is `r (round(Each_State4$TwoOrMorePer_State[1],2))*100`% of total population in `r Each_State4$State[1]`. \
Followed is the Racial Breakdown map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## Map
Race.BG.CT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
Sex.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Sex.BG, by="BG_ID_10") %>%
...
Each_State4 <- Sex.State
```
* The number of females in `r Each_State4$State[1]` is 1834373 which is `r 100*(round(Each_State4$FemalePer_State[1],2))`% of total population in `r Each_State4$State[1]`.\
* The number of males in `r Each_State4$State[1]` is 1747131 which is `r 100*(round(Each_State4$MalePer_State[1],2))`% of total population in `r Each_State4$State[1]`.\
* The ratio of number of females to males in `r Each_State4$State[1]` is `r round(Each_State4$SexRatio_State[1],2)`.\
Followed is the Gender Breakdown map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Sex.BG.CT <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Sex.BG.CT) + tm_polygons(c('Female', 'Male'), palette=list("Purples", "Reds"), title = c("Female","Male"))  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")+ tmap_options(show.messages = FALSE) 
...
HH.size.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, HH.size.BG, by="BG_ID_10")
...
Each_State4 <- HH.size.State
```
* The average household size is `r round(Each_State4$AveHouseH_State[1], 2)` in `r Each_State4$State[1]`.\
Followed is the Average Household Size map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
HH.size.BG.CT <- tidycensus::get_acs(geography = "block group",
															survey="acs5",
															state=i,
															year = endyr,
															output = "wide",
...
tm_shape(HH.size.BG.CT) + tm_polygons("AveHouseH_BG", style = "quantile", palette="Purples", border.col = "darkgray", title="Ave Household Size") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite") + tmap_options(show.messages = FALSE)
...
Educ.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Educ.BG, by="BG_ID_10")
...
Each_State4 <- Educ.State
```
* The number of people, who have less than high school degree is 235826 in `r Each_State4$State[1]` which is `r (round(Each_State4$LessThanHSPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State4$State[1]`.\

* The number of people, who have high school degree is 670519 in `r Each_State4$State[1]` which is `r (round(Each_State4$HSGradPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State4$State[1]`.\

* The number of people, who have some college degree is 607136 in `r Each_State4$State[1]` which is `r (round(Each_State4$SomeCollPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State4$State[1]`.\

* The number of people, who have bachelor degree is 538924 in `r Each_State4$State[1]` which is `r (round(Each_State4$BachPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State4$State[1]`.\

* The number of people, who have master's degree is 309751 in `r Each_State4$State[1]` which is `r (round(Each_State4$MasterPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State4$State[1]`.\

* The number of people, who have professional school degree is 74380 in `r Each_State4$State[1]` which is `r (round(Each_State4$ProfPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State4$State[1]`.\

* The number of people, who have doctorate degree is 42172 in `r Each_State4$State[1]` which is `r (round(Each_State4$DocPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State4$State[1]`.\
Followed is the Educational Attainment map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Educ.BG.CT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
Single.HH.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Single.HH.BG, by="BG_ID_10")
...
Each_State4 <- Single.HH.State

```
* The percentage of single-female-headed is `r (round(Each_State4$FemHeadPer_State[1],2))*100`% of total number of family households in `r Each_State4$State[1]`.\
* The percentage of single-male-headed is `r (round(Each_State4$MaleHeadPer_State[1],2))*100`% of total number of family households in `r Each_State4$State[1]`.\
Followed is the Single Parent Households map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Single.HH.BG.CT <- tidycensus::get_acs(geography = "block group",
															survey="acs5",
															state=i,
															year = endyr,
															output = "wide",
...
Poverty.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Poverty.BG, by="BG_ID_10")
...
Poverty.State <- tidycensus::get_acs(geography = "state",
  ...
  cbind(Poverty.BG) 
Each_State4 <- Poverty.State
```
* The percentage of family, who are in poverty is `r (round(Each_State4$FamPovPer_State[1],2))*100` % in `r Each_State4$State[1]`.\
* The percentage of population, who are in poverty is `r (round(Each_State4$PopPovPer_State[1],2))*100` % in `r Each_State4$State[1]`.\
* The total number of people, who are in poverty is 348449 in `r Each_State4$State[1]`.\
Followed is the Population in Poverty map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Poverty.BG.CT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Poverty.BG.CT) + tm_polygons("FamPovPer_BG", style="jenks", palette="Purples", border.col = "darkgray", title="Family in Poverty",popup.format = list(fun = make_percent), legend.format = list(fun = make_percent)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
...
Each_State4 <- OldAlone.State %>%
  cbind(Each_State4)
## GIVE US NA
```
* The percentage of householders, who are living alone and are older than 65 years old is `r (round(Each_State4$OldAlonePer_State[1],2))*100` % in `r Each_State4$State[1]`.\
...
* `r Each_State4$State[1]` has 204 public school district, which contain 1028 public schools. Also, `r Each_State4$State[1]` has 261 private schools.(https://nces.ed.gov/)

\


```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State4$SchoolsDistrict <- 204
Each_State4$PubSchools <- 1028
Each_State4$PvSchools <- 261
```
...
* `r Each_State4$State[1]` has 67 universities.(https://nces.ed.gov/)

```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State4$NumUni <- 67
```
...
Enrol.Sch.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Enrol.Sch.BG, by="BG_ID_10")
...
Each_State4 <- Enrol.Sch.State
```
* The total number of students, who enrolled in preschools to 12 grade levels in `r Each_State4$State[1]` schools is 636378.\
...
Num.jobs.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Num.jobs.BG, by="BG_ID_10")
...
Each_State4 <- Num.jobs.State
```
* The number of jobs within community boundaries in `r Each_State4$State[1]` is 1655193.\

Followed is the map of Number of Jobs within Community Boundaries of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}

## MAP
Num.jobs.BG.CT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Num.jobs.BG.CT) + tm_polygons("NumJobs_BG", style="jenks", palette="Reds", border.col = "darkgray", title="Number of Jobs") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
Place.Work.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Place.Work.BG, by="BG_ID_10")
...
Each_State4 <- Place.Work.State
```
* The number of people, who work in state of residence and worked in county of residence is 1301007 in `r Each_State4$State[1]`, which is `r (round(Each_State4$WorkStateCountyPer_State[1],2))*100` % of total workers in `r Each_State4$State[1]`.\
\
...
Labor.Force.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Labor.Force.BG, by="BG_ID_10")
...
Each_State4 <- Labor.Force.State
```
* The number of labor force, who are 16 years and older in `r Each_State4$State[1]` is 1944843 which is `r round(Each_State4$LabForcePer_State[1],2)` of total number of labors of which 1936354 are civilian labor forces and 8489 are armed labor forces.\

Followed is the map of Labor Force of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Labor.Force.BG.CT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Labor.Force.BG.CT) + tm_polygons("LabForce_BG", style="jenks", palette="seq", border.col = "darkgray", title="Number of Labors") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
Emp.Unemp.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Emp.Unemp.BG, by="BG_ID_10")
...
Each_State4 <- Emp.Unemp.State
```
* The rate of employment in `r Each_State4$State[1]` is `r round(Each_State4$EmpRate_State[1],2)*100`%.\
* The rate of unemployment in `r Each_State4$State[1]` is `r round(Each_State4$UnempRate_State[1],2)*100`%.\

Followed is Employed and Unemployed map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Emp.Unemp.BG.CT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Emp.Unemp.BG.CT) + tm_polygons(c("EmpRate_BG","UnempRate_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("Employment rate","Unemployment rate")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
...
Emp.NCAIS.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Emp.NCAIS.BG, by="BG_ID_10")
...
Each_State4 <- Emp.NCAIS.State
```
* Number of male by industry for the civilian employed in `r Each_State4$State[1]` is 928745 which is `r round(Each_State4$MaleNCAISPer_State[1],2)` of population, who are 16 years and over.\
* Number of female by industry for the civilian employed in `r Each_State4$State[1]` is 881700 which is `r round(Each_State4$FemaleNCAISPer_State[1],2)` of population, who are 16 years and over.\

Followed is the map of the Major Employers by NCAIS Code of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Emp.NCAIS.BG.CT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Emp.NCAIS.BG.CT) + tm_polygons(c("MaleNCAIS_BG","FemaleNCAIS_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("MaleNCAIS","FemaleNCAIS")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
```

\

```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(Emp.NCAIS.BG.CT) + tm_polygons(c("MaleNCAISPer_BG","FemaleNCAISPer_BG"), style="jenks", palette=list("Blues","Reds"), border.col = "darkgray", title=c("MaleNCAIS Per","FemaleNCAIS Per"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
```
...
Industries.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Industries.BG, by="BG_ID_10")
...
Each_State4 <- Industries.State
```
* The percentage of people, who are in agriculture, forestry, fishing, hunting, and mining in `r Each_State4$State[1]` is `r (round(Each_State4$Agriculture_State[1],2))*100`%.\

* The percentage of people, who are in construction in `r Each_State4$State[1]` state is `r (round(Each_State4$Construction_State[1],2))*100`%".\

* The percentage of people, who are in manufacturing in `r Each_State4$State[1]` state is `r (round(Each_State4$Manufacturing_State[1],2))*100`%.\

* The percentage of people, who are in wholesale trade in `r Each_State4$State[1]` state is `r (round(Each_State4$WholesaleTrade_State[1],2))*100`%.\

* The percentage of people, who are in retail trade in `r Each_State4$State[1]` state is `r (round(Each_State4$RetailTrade_State[1],2))*100`%.\

* The percentage of people, who are in transportation and warehousing, and utilities in`r Each_State4$State[1]`state is`r (round(Each_State4$Transportation_State[1],2))*100`%.\

* The percentage of people, who are in information in `r Each_State4$State[1]` state is `r (round(Each_State4$Information_State[1],2))*100`%.\

* The percentage of people, who are in finance and insurance, and real estate, and rental and leasing in `r Each_State4$State[1]` state is `r (round(Each_State4$Finance_Ins_State[1],2))*100`%.\

* The percentage of people, who are in professional, scientific, and management, and administrative, and waste management services in `r  Each_State4$State[1]` state is `r (round(Each_State4$Professional_State[1],2))*100`%.\

* The percentage of people, who are in educational services, and health care and social assistance in `r Each_State4$State[1]` state is `r (round(Each_State4$Educ_Health_SocSvcs_State[1],2))*100`%.\

* The percentage of people, who are in arts, entertainment, and recreation, and accommodation and food services in `r Each_State4$State[1]` state is `r (round(Each_State4$RecreationEtc_State[1],2))*100`%.\

* The percentage of people, who are in other services, except public administration in `r Each_State4$State[1]` state is `r (round(Each_State4$OtherIndustries_State[1],2))*100`%.\

* The percentage of people, who are in Public administration in `r Each_State4$State[1]` state is `r (round(Each_State4$PublicAdmin_State[1],2))*100`%.\
\
...
Med.Earning.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Med.Earning.BG, by="BG_ID_10")
...
Each_State4 <- Med.Earning.State
```
* The median earning in `r Each_State4$State[1]` is 41332 Dollar`.\
Followed is the Median Earning map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
# Earning at Block Group level
Med.Earning.BG.CT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Med.Earning.BG.CT) + tm_polygons("MedEarning_BG" , palette="seq", style = "jenks", title = "Median Earning", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") 
...
Emp.Age.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Emp.Age.BG, by="BG_ID_10")
...
Each_State4 <- Emp.Age.State
```
* The number of workers, who are younger than 29 years old in `r Each_State4$State[1]` is 406927 which is `r (round(Each_State4$AgeEmpU29Per_State[1],2))*100`% of total number of workers.\

* The number of workers, who are between 30 and 54 years old in `r Each_State4$State[1]` is 938589 which is `r (round(Each_State4$AgeEmp3054Per_State[1],2))*100`% of total number of workers.\

* The number of workers, who are older than 55 years old in `r Each_State4$State[1]` is 473418 which is `r (round(Each_State4$AgeEmpO55Per_State[1],2))*100`% of total number of workers.\
...
Emp.Race.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Emp.Race.BG, by="BG_ID_10")
...
Each_State4 <- Emp.Race.State 
```

* The number of white workers in `r Each_State4$State[1]` is 1426747 which is `r (round(Each_State4$WhiteEmpPer_State[1],2))*100`% of total number of workers.\

* The number of black workers in `r Each_State4$State[1]` is 177972 which is `r (round(Each_State4$BlackEmpPer_State[1],2))*100`% of total number of workers.\

* The number of asian workers in `r Each_State4$State[1]` is 83775 which is `r (round(Each_State4$AsianEmpPer_State[1],2))*100`% of total number of workers.\

* The number of hispanic or latino workers in `r Each_State4$State[1]` is 252901 which is `r (round(Each_State4$HispanicEmpPer_State[1],2))*100`% of total number of workers.\

* The number of workers, who have two or more races in `r Each_State4$State[1]` is 39357 which is `r (round(Each_State4$TwoOrMoreEmpPer_State[1],2))*100`% of total number of workers.\
\
...
Emp.Educ.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Emp.Educ.BG, by="BG_ID_10")
...
Each_State4 <- Emp.Educ.State 
```
* The number of workers, who have less than high school degree in `r Each_State4$State[1]` is 103803 which is `r (round(Each_State4$LessThanHSLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have high school degree in `r Each_State4$State[1]` is 402992 which is `r (round(Each_State4$HSGradLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have some college or associates degree in `r Each_State4$State[1]` is 432273 which is `r (round(Each_State4$SomeCollLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have bachelors degree or higher in `r Each_State4$State[1]` is 736699 which is `r (round(Each_State4$BachOrHigherLaborPer[1],2))*100`% of total number of workers.\
...
Emp.Sex.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Emp.Sex.BG, by="BG_ID_10")
...
Each_State4 <- Emp.Sex.State
```
* The number of male workers in `r Each_State4$State[1]` is 936266 which is `r (round(Each_State4$EmpMalePer[1],2))*100`% of total number of workers.\
* The number of female workers in `r Each_State4$State[1]` is 882668 which is `r (round(Each_State4$EmpFemalePer[1],2))*100`% of total number of workers.\
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State4$TotalRevenueFY19 <- 20786700000
Each_State4$REVIndividualIncomeTaxFY19 <- 9312200000
Each_State4$REVSalesUseTaxFY19 <- 4725000000
Each_State4$REVBusinessFY19 <- 1786800000
Each_State4$REVBusinessFY19 <- 1776100000
Each_State4$REVCigaretteTaxFY19 <- 501500000
Each_State4$REVGamblingFY19 <- 595300000
Each_State4$REVOtherRevenueFY19 <- 1420500000
Each_State4$REVHealthCareFY19 <- 1044100000
Each_State4$REVMortorFY19 <- 506100000
Each_State4$REVOtherTaxesFY19 <- 530600000
```
* The total revenues for `r Each_State4$State[1]` is $ 20,786,700,000 in Fiscal Year 2019, which includes sales and use tax with 4,725,000,000 Dollar, individual income tax with 9,312,200,000 Dollar, business with 1,786,800,000 Dollar, cigarette & tobacco tax with 501,500,000 Dollar, gambling with 595,300,000 Dollar, federal funds with 1,776,100,000 Dollar health provider tax by 1,044,100,000 Dollar, motor fuels by 506,100,000 Dollar, other taxes with 530,600,000 Dollar and other revenues with 1,420,500,000 Dollar. (https://https://www.cga.ct.gov/)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State4$ExpendituresFY19 <- 18790600000
Each_State4$EXEducationFY19 <- 5275000000
Each_State4$EXConserveDevelopmenFY19 <- 176900000
Each_State4$EXGeneralGovernmentFY19 <- 657900000
Each_State4$EXHumanServicesFY19 <- 4354600000
Each_State4$EXNonFunctionalFY19 <- 5917600000
Each_State4$EXProtectionFY19 <- 284900000
Each_State4$EXCorrectionFY19 <- 1367000000
Each_State4$EXHealthFY19 <- 1209600000
Each_State4$EXJudicialFY19 <- 571400000
Each_State4$EXLegislativeFY19 <- 70500000

```

* The total amount of expenditures for `r Each_State4$State[1]` is $18,790,600,000 in Fiscal Year 2019 and the major expense driver is related to education with 5,275,000,000 Dollar. Also, expenditures include non-functional with 5,917,600,000 Dollar, human services with 4,354,600,000 Dollar, corrections with 1,367,000,000 Dollar, health with 1,209,600,000 Dollar, general government with 657,900,000 Dollar, judicial with 571,400,000 Dollar, regulation and protection with 284,900,000 Dollar, Conservation and Development with 176900000 Dollar, and legislative with 70,500,000 Dollar.(https://https://www.cga.ct.gov/)
...
Snap.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Snap.BG, by="BG_ID_10")
...
Each_State4 <- Snap.State 

#######################################################################################################################################

```
* The percent of households, who received Food Stamps/SNAP in the past 12 months in `r Each_State4$State[1]` state is `r (round(Each_State4$SnapRec_State[1],2))*100`%.\
...
Housing.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Housing.BG, by="BG_ID_10")
...
Each_State4 <- Housing.State
```
* The median house value in `r Each_State4$State[1]` is 272700 Dollar.\

* The average house value in `r Each_State4$State[1]` is 377870.2 Dollar.\

* The proportion of renters to owners in `r Each_State4$State[1]` is `r round(Each_State4$RentersToOwners_State[1],2)`.\

* The percentage of renters in `r Each_State4$State[1]` is `r (round(Each_State4$RentersPer_State[1],2))*100`%.\

* The percentage of owners in `r Each_State4$State[1]` is `r (round(Each_State4$HomeOwnPer_State[1],2))*100`%.\

Followed is the map of Proportion of Renters to Owners, Median Home Values, and Average Home Values of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Housing.BG.CT <- tidycensus::get_acs(geography = "block group",
															 survey="acs5",
															 state=i,
															 year = endyr,
															 output = "wide",
...
tm_shape(Housing.BG.CT) + tm_polygons("MedianHValue_BG" , palette="Blues", style = "quantile", title = "Median Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("right","bottom"))

```
\

```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(Housing.BG.CT) + tm_polygons("AvgHValue_BG" , palette="Greens", style = "quantile", title = "Average Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("right","bottom"))
```
\

```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(Housing.BG.CT) + tm_polygons("RentersToOwners_BG" , palette="Reds", style = "jenks", title = "RentersToOwners") + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("right","bottom"))
```
...
Dur.Occ.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, Dur.Occ.BG, by="BG_ID_10")
...
Each_State4 <- Dur.Occ.State

```
* The median year that a homeowner moved into their current domicile in `r Each_State4$State[1]` is `r Each_State4$MedYrOwnMovedIn_State[1]`.\
* The median year that a renter moved into their current domicile in `r Each_State4$State[1]` is `r Each_State4$MedYrRentMovedIn_State[1]`.\
Followed is the map of Duration of Occupancy of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Dur.Occ.BG.CT <- tidycensus::get_acs(geography = "block group",
															 survey="acs5",
															 state=i,
															 year = endyr,
															 output = "wide",
...
tm_shape(Dur.Occ.BG.CT) + tm_polygons(c("MedYrMovedIn_BG","MedYrRentMovedIn_BG","MedYrOwnMovedIn_BG"), style="jenks", palette=list("Greens","seq","Blues"), border.col = "darkgray", title=c("MedYrMovedIn","MedYrRentMovedIn","MedYrOwnMovedIn")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
```
...
CostBurdened.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State4, CostBurdened.BG, by="BG_ID_10")
...
Each_State4 <- CostBurdened.State
```
* The number of households, who spend 30 percent or more on housing costs in `r Each_State4$State[1]` is 259032 which is `r (round(Each_State4$CostBurdenedPer[1],3))*100`% of total number of households, who spend money on housing costs.\
