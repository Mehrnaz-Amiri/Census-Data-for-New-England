...
In this report, 30 demographic, educational institutions, employment, municipal finances, food insecurity and housing variables are presented using federally-published data sources for Vermont state. To better demonstrate their ties to geography of the state, the variables have been mapped on the block group level. \
...
Each_State5 <- Pop.State
```


The total population of `r Each_State5$State[1]` is 624977. Followed is the popuation size map of the state.\
\

```{r , echo=FALSE, warning=FALSE, message=FALSE, comment=""}
## MAP
Pop.BG.VT <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Pop.BG.VT) + tm_polygons('TotalPop_BG' , palette="seq", style = "jenks", title = "Total Population")  + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")
```
\
...
Age.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, by="BG_ID_10") %>%
...
Each_State5 <- Age.State
```
* The number of people, who are younger than 18 Years old is 118346 in `r Each_State5$State[1]` 
, which is `r (round(Each_State5$AgeU18Per_State[1],2))*100`% of total population in `r Each_State5$State[1]`. \
* The number of people, who are between 18 and 34 years old is 139750 in `r Each_State5$State[1]`, which is `r (round(Each_State5$Age1834Per_State[1],2))*100`% of total population in `r Each_State5$State[1]`.\
* The number of people, who are between 35 and 64 years old is 253331 in `r Each_State5$State[1]`, which is `r (round(Each_State5$Age3564Per_State[1],2))*100`% of total population in `r Each_State5$State[1]`.\
* The number of people, who are older than 65 years old is 113550 in `r Each_State5$State[1]`, which is `r (round(Each_State5$AgeO65Per_State[1],2))*100`% of total population in `r Each_State5$State[1]`.\

Followed is the popuation age map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## Map
Age.BG.VT <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Age.BG.VT) + tm_polygons(c('AgeU18Per_BG','Age1834Per_BG',"Age3564Per_BG","AgeO65Per_BG") , palette=list("Oranges", "Greens", "Blues", "Purples") ,style = "quantile", title = c('AgeU18','Age1834',"Age3564","AgeO65"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray") + tmap_options(show.messages = FALSE) + tm_layout(legend.position = c("right","bottom"))
...
Each_State5 <- Income.State
```
* The median household income in the past 12 months is 60076 Dollar in `r Each_State5$State[1]`.\
Followed is the Median Household Income map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Income.BG.VT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
Race.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Race.BG, by="BG_ID_10") %>%
...
Each_State5 <- Race.State
```
* The number of people who are white in `r Each_State5$State[1]` is 581023 which is `r (round(Each_State5$WhitePer_State[1],2))*100`% of total population in `r Each_State5$State[1]`.\
* The number of people who are black in `r Each_State5$State[1]` is 7602 which is `r (round(Each_State5$BlackPer_State[1],2))*100`% of total population in `r Each_State5$State[1]`.\
* The number of people who are asian in `r Each_State5$State[1]` is 10541 which is `r (round(Each_State5$AsianPer_State[1],2))*100`% of total population in `r Each_State5$State[1]`.\
* The number of people who are hispanic in `r Each_State5$State[1]` is 11677 which is `r (round(Each_State5$HispanicPer_State[1],2))*100`% of total population in `r Each_State5$State[1]`.\
* The number of people who are have two or more races in `r Each_State5$State[1]` is 11199 which is `r (round(Each_State5$TwoOrMorePer_State[1],2))*100`% of total population in `r Each_State5$State[1]`. \
Followed is the Racial Breakdown map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## Map
Race.BG.VT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
Sex.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Sex.BG, by="BG_ID_10") %>%
...
Each_State5 <- Sex.State
```
* The number of females in `r Each_State5$State[1]` is 317004 which is `r 100*(round(Each_State5$FemalePer_State[1],2))`% of total population in `r Each_State5$State[1]`.\
* The number of males in `r Each_State5$State[1]` is 307973 which is `r 100*(round(Each_State5$MalePer_State[1],2))`% of total population in `r Each_State5$State[1]`.\
* The ratio of number of females to males in `r Each_State5$State[1]` is `r round(Each_State5$SexRatio_State[1],2)`.\
Followed is the Gender Breakdown map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Sex.BG.VT <- tidycensus::get_acs(geography = "block group",
													 survey="acs5",
													 state=i,
													 year = endyr,
													 output = "wide",
...
library(tmap)
tm_shape(Sex.BG.VT) + tm_polygons(c('FemalePer', 'MalePer'), palette=list("Blues", "Oranges"), title = c("Female","Male"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")+ tmap_options(show.messages = FALSE) + tm_layout(legend.position = c("right","bottom"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
library(tmap)
tm_shape(Sex.BG.VT) + tm_polygons(c('Female', 'Male'), palette=list("Purples", "Reds"), title = c("Female","Male"))  + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") +tm_borders(col = "darkgray")+ tmap_options(show.messages = FALSE) + tm_layout(legend.position = c("right","bottom"))
```
\
...
HH.size.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, HH.size.BG, by="BG_ID_10")
...
Each_State5 <- HH.size.State
```
* The average household size is `r round(Each_State5$AveHouseH_State[1], 2)` in `r Each_State5$State[1]`.\
Followed is the Average Household Size map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
HH.size.BG.VT <- tidycensus::get_acs(geography = "block group",
															survey="acs5",
															state=i,
															year = endyr,
															output = "wide",
...
tm_shape(HH.size.BG.VT) + tm_polygons("AveHouseH_BG", style = "quantile", palette="Purples", border.col = "darkgray", title="Ave Household Size") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite") + tmap_options(show.messages = FALSE)
...
Educ.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Educ.BG, by="BG_ID_10")
...
Each_State5 <- Educ.State
```
* The number of people, who have less than high school degree is 32707 in `r Each_State5$State[1]` which is `r (round(Each_State5$LessThanHSPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State5$State[1]`.\

* The number of people, who have high school degree is 128172 in `r Each_State5$State[1]` which is `r (round(Each_State5$HSGradPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State5$State[1]`.\

* The number of people, who have some college degree is 114710 in `r Each_State5$State[1]` which is `r (round(Each_State5$SomeCollPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State5$State[1]`.\

* The number of people, who have bachelor degree is 97034 in `r Each_State5$State[1]` which is `r (round(Each_State5$BachPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State5$State[1]`.\

* The number of people, who have master's degree is 47484 in `r Each_State5$State[1]` which is `r (round(Each_State5$MasterPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State5$State[1]`.\

* The number of people, who have professional school degree is 11287 in `r Each_State5$State[1]` which is `r  (round(Each_State5$ProfPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State5$State[1]`.\

* The number of people, who have doctorate degree is 8301 in `r Each_State5$State[1]` which is `r (round(Each_State5$DocPer_State[1],2))*100`% of the population who are 25 years old or older in `r Each_State5$State[1]`.\
Followed is the Educational Attainment map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Educ.BG.VT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
Single.HH.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Single.HH.BG, by="BG_ID_10")
...
Each_State5 <- Single.HH.State

```
* The percentage of single-female-headed is `r (round(Each_State5$FemHeadPer_State[1],2))*100`% of total number of family households in `r Each_State5$State[1]`.\
* The percentage of single-male-headed is `r (round(Each_State5$MaleHeadPer_State[1],2))*100`% of total number of family households in `r Each_State5$State[1]`.\
Followed is the Single Parent Households map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Single.HH.BG.VT <- tidycensus::get_acs(geography = "block group",
															survey="acs5",
															state=i,
															year = endyr,
															output = "wide",
...
Poverty.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Poverty.BG, by="BG_ID_10")
...
Poverty.State <- tidycensus::get_acs(geography = "state",
  ...
  cbind(Poverty.BG) 
Each_State5 <- Poverty.State
```
* The percentage of family, who are in poverty is `r (round(Each_State5$FamPovPer_State[1],2))*100` % in `r Each_State5$State[1]`.\
* The percentage of population, who are in poverty is `r (round(Each_State5$PopPovPer_State[1],2))*100` % in `r Each_State5$State[1]`.\
* The total number of people, who are in poverty is 67034 in `r Each_State5$State[1]`.\
Followed is the Population in Poverty map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Poverty.BG.VT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Poverty.BG.VT) + tm_polygons("FamPovPer_BG", style="jenks", palette="Purples", border.col = "darkgray", title="Family in Poverty",popup.format = list(fun = make_percent), legend.format = list(fun = make_percent)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
...
Each_State5 <- OldAlone.State %>%
  cbind(Each_State5)
## GIVE US NA
```
* The percentage of householders, who are living alone and are older than 65 years old is `r (round(Each_State5$OldAlonePer_State[1],2))*100` % in `r Each_State5$State[1]`.\
...
* `r Each_State5$State[1]` has 235 public school district, which contain 312 public schools. Also, `r Each_State5$State[1]` has 83 private schools.(https://nces.ed.gov/)

\


```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State5$SchoolsDistrict <- 235
Each_State5$PubSchools <- 312
Each_State5$PvSchools <- 83
```
...
* `r Each_State5$State[1]` has 21 universities.(https://nces.ed.gov/)

```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State5$NumUni <- 21
```
...
Enrol.Sch.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Enrol.Sch.BG, by="BG_ID_10")
...
Each_State5 <- Enrol.Sch.State
```
* The total number of students, who enrolled in preschools to 12 grade levels in `r Each_State5$State[1]` schools is 98291.\
...
Num.jobs.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Num.jobs.BG, by="BG_ID_10")
...
Each_State5 <- Num.jobs.State
```
* The number of jobs within community boundaries in `r Each_State5$State[1]` is 300104.\

Followed is the map of Number of Jobs within Community Boundaries of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}

## MAP
Num.jobs.BG.VT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Num.jobs.BG.VT) + tm_polygons("NumJobs_BG", style="jenks", palette="Reds", border.col = "darkgray", title="Number of Jobs") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
Place.Work.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Place.Work.BG, by="BG_ID_10")
...
Each_State5 <- Place.Work.State

```
* The number of people, who work in state of residence and worked in county of residence is 248358 in `r Each_State5$State[1]`, which is `r (round(Each_State5$WorkStateCountyPer_State[1],2))*100` % of total workers in `r Each_State5$State[1]`.\
\
...
Labor.Force.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Labor.Force.BG, by="BG_ID_10")
...
Each_State5 <- Labor.Force.State
```
* The number of labor force, who are 16 years and older in `r Each_State5$State[1]` is 343168 which is `r round(Each_State5$LabForcePer_State[1],2)` of total number of labors of which 342620 are civilian labor forces and 548 are armed labor forces.\

Followed is the map of Labor Force of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Labor.Force.BG.VT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Labor.Force.BG.VT) + tm_polygons("LabForce_BG", style="jenks", palette="seq", border.col = "darkgray", title="Number of Labors") + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"), basemaps = "Stamen.TonerLite")
```
...
Emp.Unemp.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Emp.Unemp.BG, by="BG_ID_10")
...
Each_State5 <- Emp.Unemp.State
```
* The rate of employment in `r Each_State5$State[1]` is `r round(Each_State5$EmpRate_State[1],2)*100`%.\
* The rate of unemployment in `r Each_State5$State[1]` is `r round(Each_State5$UnempRate_State[1],2)*100`%.\

Followed is Employed and Unemployed map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Emp.Unemp.BG.VT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Emp.Unemp.BG.VT) + tm_polygons(c("EmpRate_BG","UnempRate_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("Employment rate","Unemployment rate")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
...
Emp.NCAIS.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Emp.NCAIS.BG, by="BG_ID_10")
...
Each_State5 <- Emp.NCAIS.State
```
* Number of male by industry for the civilian employed in `r Each_State5$State[1]` is 166460 which is `r round(Each_State5$MaleNCAISPer_State[1],2)` of population, who are 16 years and over.\
* Number of female by industry for the civilian employed in `r Each_State5$State[1]` is 162282 which is `r round(Each_State5$FemaleNCAISPer_State[1],2)` of population, who are 16 years and over.\

Followed is the map of the Major Employers by NCAIS Code of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
Emp.NCAIS.BG.VT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Emp.NCAIS.BG.VT) + tm_polygons(c("MaleNCAIS_BG","FemaleNCAIS_BG"), style="jenks", palette=list("Greens","seq"), border.col = "darkgray", title=c("MaleNCAIS","FemaleNCAIS")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(Emp.NCAIS.BG.VT) + tm_polygons(c("MaleNCAISPer_BG","FemaleNCAISPer_BG"), style="jenks", palette=list("Reds","Blues"), border.col = "darkgray", title=c("MaleNCAIS Per","FemaleNCAIS Per"), popup.format = list(fun = make_percent), legend.format = list(fun = make_percent)) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
```
...
Industries.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Industries.BG, by="BG_ID_10")
...
Each_State5 <- Industries.State
```
* The percentage of people, who are in agriculture, forestry, fishing, hunting, and mining in `r Each_State5$State[1]` is `r (round(Each_State5$Agriculture_State[1],2))*100`%.\

* The percentage of people, who are in construction in `r Each_State5$State[1]` state is `r (round(Each_State5$Construction_State[1],2))*100`%".\

* The percentage of people, who are in manufacturing in `r Each_State5$State[1]` state is `r (round(Each_State5$Manufacturing_State[1],2))*100`%.\

* The percentage of people, who are in wholesale trade in `r Each_State5$State[1]` state is `r (round(Each_State5$WholesaleTrade_State[1],2))*100`%.\

* The percentage of people, who are in retail trade in `r Each_State5$State[1]` state is `r (round(Each_State5$RetailTrade_State[1],2))*100`%.\

* The percentage of people, who are in transportation and warehousing, and utilities in`r Each_State5$State[1]`state is`r (round(Each_State5$Transportation_State[1],2))*100`%.\

* The percentage of people, who are in information in `r Each_State5$State[1]` state is `r (round(Each_State5$Information_State[1],2))*100`%.\

* The percentage of people, who are in finance and insurance, and real estate, and rental and leasing in `r Each_State5$State[1]` state is `r (round(Each_State5$Finance_Ins_State[1],2))*100`%.\

* The percentage of people, who are in professional, scientific, and management, and administrative, and waste management services in `r  Each_State5$State[1]` state is `r (round(Each_State5$Professional_State[1],2))*100`%.\

* The percentage of people, who are in educational services, and health care and social assistance in `r Each_State5$State[1]` state is `r (round(Each_State5$Educ_Health_SocSvcs_State[1],2))*100`%.\

* The percentage of people, who are in arts, entertainment, and recreation, and accommodation and food services in `r Each_State5$State[1]` state is `r (round(Each_State5$RecreationEtc_State[1],2))*100`%.\

* The percentage of people, who are in other services, except public administration in `r Each_State5$State[1]` state is `r (round(Each_State5$OtherIndustries_State[1],2))*100`%.\

* The percentage of people, who are in Public administration in `r Each_State5$State[1]` state is `r (round(Each_State5$PublicAdmin_State[1],2))*100`%.\
\
...
Med.Earning.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Med.Earning.BG, by="BG_ID_10")
...
Each_State5 <- Med.Earning.State
```
* The median earning in `r Each_State5$State[1]` is 32376 Dollar.\
Followed is the Median Earning map of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="",results='hide'}
## MAP
# Earning at Block Group level
Med.Earning.BG.VT <- tidycensus::get_acs(geography = "block group",
														survey="acs5",
														state=i,
														year = endyr,
														output = "wide",
...
tm_shape(Med.Earning.BG.VT) + tm_polygons("MedEarning_BG" , palette="seq", style = "jenks", title = "Median Earning", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,view.legend.position = c("right", "bottom"),basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") 
...
Emp.Age.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Emp.Age.BG, by="BG_ID_10")
...
Each_State5 <- Emp.Age.State
```
* The number of workers, who are younger than 29 years old in `r Each_State5$State[1]` is 76507 which is `r (round(Each_State5$AgeEmpU29Per_State[1],2))*100`% of total number of workers.\

* The number of workers, who are between 30 and 54 years old in `r Each_State5$State[1]` is 159903 which is `r (round(Each_State5$AgeEmp3054Per_State[1],2))*100`% of total number of workers.\

* The number of workers, who are older than 55 years old in `r Each_State5$State[1]` is 92880 which is `r (round(Each_State5$AgeEmpO55Per_State[1],2))*100`% of total number of workers.\
...
Emp.Race.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Emp.Race.BG, by="BG_ID_10")
...
Each_State5 <- Emp.Race.State 
```

* The number of white workers in `r Each_State5$State[1]` is 313313 which is `r (round(Each_State5$WhiteEmpPer_State[1],2))*100`% of total number of workers.\

* The number of black workers in `r Each_State5$State[1]` is 7602 which is `r (round(Each_State5$BlackEmpPer_State[1],2))*100`% of total number of workers.\

* The number of asian workers in `r Each_State5$State[1]` is 5707 which is `r (round(Each_State5$AsianEmpPer_State[1],2))*100`% of total number of workers.\

* The number of hispanic or latino workers in `r Each_State5$State[1]` is 5524 which is `r (round(Each_State5$HispanicEmpPer_State[1],2))*100`% of total number of workers.\

* The number of workers, who have two or more races in `r Each_State5$State[1]` is 4135 which is `r (round(Each_State5$TwoOrMoreEmpPer_State[1],2))*100`% of total number of workers.\
\
...
Emp.Educ.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Emp.Educ.BG, by="BG_ID_10")
...
Each_State5 <- Emp.Educ.State 
```
* The number of workers, who have less than high school degree in `r Each_State5$State[1]` is 12923 which is `r (round(Each_State5$LessThanHSLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have high school degree in `r Each_State5$State[1]` is 78765 which is `r (round(Each_State5$HSGradLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have some college or associates degree in `r Each_State5$State[1]` is 78531 which is `r (round(Each_State5$SomeCollLaborPer[1],2))*100`% of total number of workers.\

* The number of workers, who have bachelors degree or higher in `r Each_State5$State[1]` is 121870 which is `r (round(Each_State5$BachOrHigherLaborPer[1],2))*100`% of total number of workers.\
...
Emp.Sex.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Emp.Sex.BG, by="BG_ID_10")
...
Each_State5 <- Emp.Sex.State

```
* The number of male workers in `r Each_State5$State[1]` is 166977 which is `r (round(Each_State5$EmpMalePer[1],2))*100`% of total number of workers.\
* The number of female workers in `r Each_State5$State[1]` is 162313 which is `r (round(Each_State5$EmpFemalePer[1],2))*100`% of total number of workers.\
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
# GENERAL
Each_State5$TotalRevenueFY19 <- 5906800000
Each_State5$REVSevicesFY19 <- 434600000
Each_State5$REVOperatinGrantFY19 <- 1860700000
Each_State5$REVCapitalGrantFY19 <- 175200000
Each_State5$REVSalesUseTaxFY19 <- 416800000
Each_State5$REVMealsRoomsTaxFY19 <- 184600000
Each_State5$REVOtherTaxesFY19 <- 653700000
Each_State5$REVEducationTaxFY19 <- 1105500000
Each_State5$REVMiscellaneousFY19 <- 30300000
Each_State5$REVIndividualIncomeTaxFY19 <- 1045400000
```
* The total revenues for `r Each_State5$State[1]` is $ 5,906,800,000 in Fiscal Year 2019, which includes sales and use tax with 416,800,000 Dollar, meals and rooms with 184,600,000 Dollar, individual income tax with 1,045,400,000 Dollar, charge for services with 434,600,000 Dollar, operating grants with 1,860,700,000 Dollar, capital grants with 175,200,000 Dollar, statewide education tax with 1,105,500,000 Dollar, miscellaneous with 30,300,000 Dollar, and other taxes 653,700,000 Dollar.(https://finance.vermont.gov/)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
Each_State5$ExpendituresFY19 <- 5877000000
Each_State5$EXGeneralGovernmentFY19 <- 159700000
Each_State5$EXProtectionFY19 <- 326000000
Each_State5$EXHumanServicesFY19 <- 2539000000
Each_State5$EXLaborFY19 <- 31100000
Each_State5$EXEducationFY19 <- 2157200000
Each_State5$EXNaturalResourcesFY19 <- 143800000
Each_State5$EXEconDevelopmenFY19 <- 41800000
Each_State5$EXTransportationFY19 <- 460600000
Each_State5$DebtFY19 <- 17800000
```

* The total amount of expenditures for `r Each_State5$State[1]` is $5,877,000,000 in Fiscal Year 2019 and the major expense driver is related to human services with 2,539,000,000 Dollar. Also, expenditures include education with 2,157,200,000 Dollar, general government with 159,700,000 Dollar, protection to persons and property with 326,000,000 Dollar, labor with 31,100,000 Dollar, natural resources with 143,800,000 Dollar, commerce and community development with 41,800,000 Dollar, transportation with 460,600,000 dollar, and interest on long-term debt with 17,800,000 Dollar.(https://finance.vermont.gov/)
...
Snap.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Snap.BG, by="BG_ID_10")
...
Each_State5 <- Snap.State 

#######################################################################################################################################

```
* The percent of households, who received Food Stamps/SNAP in the past 12 months in `r Each_State5$State[1]` state is `r (round(Each_State5$SnapRec_State[1],2))*100`%.\
...
Housing.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Housing.BG, by="BG_ID_10")
...
Each_State5 <- Housing.State
```
* The median house value in `r Each_State5$State[1]` is 223700 Dollar.\

* The average house value in `r Each_State5$State[1]` is 259708.1 Dollar.\

* The proportion of renters to owners in `r Each_State5$State[1]` is `r round(Each_State5$RentersToOwners_State[1],2)`.\

* The percentage of renters in `r Each_State5$State[1]` is `r (round(Each_State5$RentersPer_State[1],2))*100`%.\

* The percentage of owners in `r Each_State5$State[1]` is `r (round(Each_State5$HomeOwnPer_State[1],2))*100`%.\

Followed is the map of Proportion of Renters to Owners, Median Home Values, and Average Home Values of the state.\

```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Housing.BG.VT <- tidycensus::get_acs(geography = "block group",
															 survey="acs5",
															 state=i,
															 year = endyr,
															 output = "wide",
...
tm_shape(Housing.BG.VT) + tm_polygons("MedianHValue_BG" , palette="Blues", style = "quantile", title = "Median Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("right","bottom"))

tm_shape(Housing.BG.VT) + tm_polygons("AvgHValue_BG" , palette="Greens", style = "quantile", title = "Average Home Value", popup.format = list(fun = make_dollar), legend.format = list(fun = make_dollar)) + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray")+tm_layout(legend.position = c("right","bottom"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', comment=""}
tm_shape(Housing.BG.VT) + tm_polygons("RentersToOwners_BG" , palette="Reds", style = "jenks", title = "RentersToOwners") + tm_view(alpha = 0.85,basemaps = "Stamen.TonerLite") + tm_borders(col = "darkgray") + tm_layout(legend.position = c("right","bottom"))
```
...
Dur.Occ.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, Dur.Occ.BG, by="BG_ID_10")
...
Each_State5 <- Dur.Occ.State

```
* The median year that a homeowner moved into their current domicile in `r Each_State5$State[1]` is `r Each_State5$MedYrOwnMovedIn_State[1]`.\
* The median year that a renter moved into their current domicile in `r Each_State5$State[1]` is `r Each_State5$MedYrRentMovedIn_State[1]`.\
Followed is the map of Duration of Occupancy of the state.\
```{r echo=FALSE, warning=FALSE, message=FALSE, comment="", results='hide'}
## MAP
Dur.Occ.BG.VT <- tidycensus::get_acs(geography = "block group",
															 survey="acs5",
															 state=i,
															 year = endyr,
															 output = "wide",
...
tm_shape(Dur.Occ.BG.VT) + tm_polygons(c("MedYrMovedIn_BG","MedYrRentMovedIn_BG","MedYrOwnMovedIn_BG"), style="jenks", palette=list("Greens","seq","Blues"), border.col = "darkgray", title=c("MedYrMovedIn","MedYrRentMovedIn","MedYrOwnMovedIn")) + tm_view(alpha = 0.85, basemaps = "Stamen.TonerLite") + tm_layout(legend.position = c("right","bottom"))
```
...
CostBurdened.BG <- tidycensus::get_acs(geography = "block group",
  ...
  left_join(Each_State5, CostBurdened.BG, by="BG_ID_10")
...
Each_State5 <- CostBurdened.State
```
* The number of households, who spend 30 percent or more on housing costs in `r Each_State5$State[1]` is 52760 which is `r (round(Each_State5$CostBurdenedPer[1],3))*100`% of total number of households, who spend money on housing costs.\
